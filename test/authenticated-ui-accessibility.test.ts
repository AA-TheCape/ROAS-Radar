...
import type * as AxeModule from "../dashboard/node_modules/axe-core";
import type * as JsdomModule from "../dashboard/node_modules/jsdom";
import type * as ReactModule from "../dashboard/node_modules/react";
import type * as ReactDomModule from "../dashboard/node_modules/react-dom";
import type * as ReactDomClientModule from "../dashboard/node_modules/react-dom/client";
import type * as ReactDomServerModule from "../dashboard/node_modules/react-dom/server";
...
const React = requireFromDashboard("react") as typeof ReactModule;
const { renderToStaticMarkup } = requireFromDashboard(
	"react-dom/server",
) as typeof ReactDomServerModule;
const { createRoot } = requireFromDashboard(
	"react-dom/client",
) as typeof ReactDomClientModule;
const { flushSync } = requireFromDashboard(
	"react-dom",
) as typeof ReactDomModule;
const axe = requireFromDashboard("axe-core") as typeof AxeModule;
const { JSDOM } = requireFromDashboard("jsdom") as typeof JsdomModule;
...
test("modal traps focus, closes on escape, and restores the previous focus target", async () => {
	const dom = createDom();
	const { Modal, Button } = await loadModule<
	...
	assert.equal(
		(dom.window.document.activeElement as HTMLElement | null)?.getAttribute(
			"aria-label",
		),
		"Close modal",
	);
