...
test('recoverShopifyAttributionHints applies click-id-backed synthetic attribution only for eligible web orders without deterministic matches', async () => {
  ...
  try {
    ...
    const recovery = await shopifyTestUtils.recoverShopifyAttributionHints('UTC', '2026-04-08', '2026-04-08');
    ...
    const attributionResult = await fetchAttributionResult('order-shopify-hint-click-id-1');
...
test('recoverShopifyAttributionHints ignores non-web orders even when Shopify hints are present', async () => {
  ...
  try {
    ...
    assert.equal(await fetchAttributionResult('order-shopify-hint-pos-1'), null);
    assert.equal(await fetchOrderSnapshot('order-shopify-hint-pos-1'), null);
    assert.equal(await fetchPendingAttributionJobs('order-shopify-hint-pos-1'), 0);
  } finally {
    await resetIntegrationDatabase();
  }
});
...
test('recoverShopifyAttributionHints suppresses Shopify fallback when checkout or cart evidence resolves a deterministic session', async () => {
  await resetIntegrationDatabase();
  const { pool, processAttributionQueue, shopifyTestUtils } = await getModules();
  ...
  try {
    ...
    await insertShopifyOrder(pool, {
      ...
      rawPayload: JSON.stringify({
        ...
        landing_site: 'https://store.example/products/widget?utm_source=klaviyo&utm_medium=email',
        note_attributes: [{ name: 'fbclid', value: 'FB-CLICK-SHOULD-NOT-WIN' }],
        line_items: []
      })
    });

    const recovery = await shopifyTestUtils.recoverShopifyAttributionHints('UTC', '2026-04-09', '2026-04-09');
    assert.deepEqual(recovery, {
      rescannedOrders: 1,
      relinkedOrders: 1,
      requeuedOrders: 1,
    ...
    assert.equal(await fetchAttributionResult('order-shopify-hint-suppressed-1'), null);
    assert.equal(await fetchPendingAttributionJobs('order-shopify-hint-suppressed-1'), 1);
    ...
    const attributionResult = await fetchAttributionResult('order-shopify-hint-suppressed-1');
    assert.deepEqual(attributionResult, {
