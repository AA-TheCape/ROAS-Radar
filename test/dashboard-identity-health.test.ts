...
import { changeInputValue, click, h, loadDashboardModule, mountUi, tick } from './dashboard-ui-test-helpers';
...
test('identity graph health view exposes filters, KPIs, and conflict drill-down selection', async () => {
  ...
  const filterUpdates: Array<{ startDate: string; endDate: string; source?: string }> = [];
  let refreshCount = 0;
  ...
  const mounted = await mountUi(
    h(IdentityGraphHealthView, {
      ...
      onFiltersChange: (next: { startDate: string; endDate: string; source?: string }) => {
        ...
        data: {
          ...
          conflicts: [
            ...
            },
            {
              ...
              conflictCode: 'hashed_email_conflicts_across_authoritative_customers',
  ...
  try {
    ...
    const refreshButton = Array.from(mounted.container.querySelectorAll('button')).find((button) =>
      /Refresh health metrics/.test(button.textContent ?? '')
    );
    ...
    const [startDateInput] = Array.from(mounted.container.querySelectorAll<HTMLInputElement>('input[type="date"]'));
    assert.ok(startDateInput);

    const sourceSelect = mounted.container.querySelector('select') as HTMLSelectElement;
    ...
    assert.match(mounted.container.textContent ?? '', /Hashed Email Conflicts Across Authoritative Customers/);
