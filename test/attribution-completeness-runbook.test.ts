...
const runbookPath = path.join(__dirname, '..', 'docs', 'runbooks', 'attribution-completeness.md');
...
test('attribution completeness runbook documents the Shopify recovery order and backfill options', () => {
  ...
  assert.match(text, /Always run a dry run first for the exact same date window before queueing a write-enabled run\./);
