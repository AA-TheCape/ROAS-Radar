...
function buildRawPayload(attributes: Array<{ key: string; value: string }> = []): string {
...
async function insertSessionAttributionIdentity(pool: Pool, sessionId: string): Promise<void> {
  await pool.query(
    `
...
async function seedCanonicalOrder(pool: Pool, shopifyOrderId: string, sessionId: string, rawPayload?: string): Promise<void> {
...
test('enqueueShopifyOrderWriteback upserts one queue row per order idempotently', async () => {
  ...
  await enqueueShopifyOrderWriteback('1001', 'reconciliation_missing_canonical_attributes');

  const result = await pool.query<{ total: string; requested_reason: string; queue_key: string; status: string }>(
    `
      SELECT
        COUNT(*)::text AS total,
  ...
  assert.equal(result.rows[0].requested_reason, 'reconciliation_missing_canonical_attributes');
...
test('processShopifyOrderWritebackQueue builds canonical attributes and completes claimed jobs', async () => {
  const { pool, enqueueShopifyOrderWriteback, processShopifyOrderWritebackQueue, testUtils } = await getModules();
  ...
  assert.deepEqual(appliedWritebacks[0].attributes, buildCanonicalShopifyAttributes(sessionId));
...
test('processShopifyOrderWritebackQueue marks transient failures for retry', async () => {
  ...
  testUtils.setWritebackProcessor(async () => {
    throw Object.assign(new Error('shopify temporarily unavailable'), { retryable: true, statusCode: 503 });
  });
  ...
  const queueState = await pool.query<{ status: string; attempts: number; last_error: string | null }>(
    `
  ...
  assert.equal(queueState.rows[0].last_error, 'shopify temporarily unavailable');
});
...
test('processShopifyOrderWritebackQueue dead-letters terminal failures with retained payload context', async () => {
  ...
  testUtils.setWritebackProcessor(async () => {
    throw Object.assign(new Error('shopify rejected note attributes'), { retryable: false, statusCode: 422 });
  });
  ...
  const queueState = await pool.query<{ id: string; status: string; attempts: number; dead_lettered_at: Date | null }>(
    `
...
test('reconcileRecentShopifyOrderAttributes reports up-to-date, skipped, and failed orders separately', async () => {
  ...
  await seedCanonicalOrder(pool, '2001', needsWritebackSession, buildRawPayload());
  await seedCanonicalOrder(pool, '2002', upToDateSession, buildRawPayload(buildCanonicalShopifyAttributes(upToDateSession)));
  await insertShopifyOrder(pool, {
    shopifyOrderId: '2003',
    sessionId: null,
