...
test('identity admin health route rejects unauthorized requests', async () => {
  ...
  try {
    const { response, body } = await requestJson(server, '/api/admin/identity/health?startDate=2026-04-01&endDate=2026-04-05');
...
test('identity admin health route returns merge KPIs, daily series, and latest backfill status', async () => {
  ...
  pool.query = (async (text: string, params?: unknown[]) => {
    ...
    if (/COUNT\(\*\)::bigint AS total_ingestions/.test(text)) {
      assert.deepEqual(params, ['2026-04-01', '2026-04-05', 'shopify_order_webhook']);
      return {
        rows: [
          {
            total_ingestions: 14,
    ...
    if (/to_char\(date_trunc\('day', runs.source_timestamp AT TIME ZONE 'UTC'\), 'YYYY-MM-DD'\)/.test(text)) {
      return {
        rows: [
          {
    ...
    if (/COUNT\(\*\) FILTER \(WHERE sessions.identity_journey_id IS NULL\)::bigint AS unlinked_sessions/.test(text)) {
      return {
        rows: [
          {
            unlinked_sessions: 5,
    ...
    if (/COUNT\(\*\) FILTER \(WHERE runs.status = 'processing'\)::bigint AS active_runs/.test(text)) {
      return {
        rows: [
          {
            active_runs: 1,
    ...
    if (/FROM identity_graph_backfill_runs runs/.test(text) && /LIMIT 1/.test(text)) {
      return {
        rows: [
          {
