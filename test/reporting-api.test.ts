...
test('reporting campaigns resolve unmapped raw Meta campaign IDs from platform entity metadata', async () => {
  pool.query = (async (text: string, params?: unknown[]) => {
    if (text.includes('FROM daily_reporting_metrics')) {
      assert.deepEqual(params, ['2026-04-01', '2026-04-10', 'last_touch', 1]);
        ...
        rows: [
          {
            ...
            object_name: 'TD_Remarketing_Fender_Covers',
            parent_campaign_id: null,
            parent_campaign_name: null,
            last_seen_at: new Date('2026-04-09T12:00:00.000Z'),
            metadata_source: 'ad_platform_entity_metadata'
    ...
    assert.deepEqual(body.rows, [
      {
        ...
        revenue: 120,
        conversionRate: 2 / 14,
        campaignDisplayName: 'TD_Remarketing_Fender_Covers',
...
test('reporting orders filters and serializes Meta platform-reported attribution separately from GA4 fallback', async () => {
  pool.query = (async (text: string, params?: unknown[]) => {
    ...
    if (text.includes('WITH google_candidates AS')) {
      return { rows: [], rowCount: 0 };
    }
...
test('reporting order details expose attribution tier metadata additively', async () => {
  pool.query = (async (text: string, params?: unknown[]) => {
        ...
        rows: [
          {
            ...
            result_attribution_model: 'last_touch',
            result_session_id: '33333333-3333-4333-8333-333333333333',
            result_attributed_source: 'google',
            result_attributed_medium: 'cpc',
            result_attributed_campaign: '555',
            result_attributed_content: 'hero',
            result_attributed_term: 'widget',
            result_attributed_click_id_type: 'gclid',
            result_attributed_click_id_value: 'gclid-123',
            meta_attribution_evidence_id: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
    ...
    if (text.includes('WITH google_candidates')) {
      assert.deepEqual(params, ['2026-04-10', '2026-04-10', ['555'], 'google']);
      return {
        rows: [
          {
            campaign: '555',
  ...
  try {
    ...
    assert.equal(body.order.attributedCampaign, '555');
