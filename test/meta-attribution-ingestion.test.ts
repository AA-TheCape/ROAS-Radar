...
test('meta attribution evidence route rejects invalid payloads and emits an audit log entry', async () => {
  ...
  pool.query = (async (text: string) => {
    queryCalls += 1;
    assert.match(text, /FROM shopify_orders/);
...
test('meta attribution evidence route persists normalized payloads with canonical eligibility flags', async () => {
  ...
  pool.connect = (async () => ({
    query: async (text: string | { text: string; values?: unknown[] }, params?: unknown[]) => {
      const normalizedText = (typeof text === 'string' ? text : text.text).trim();
  ...
  try {
    ...
    assert.equal(successLog?.sourceKind, 'order_joinable');
    assert.equal(successLog?.matchBasis, 'fbc');
    assert.equal(successLog?.confidenceBucket, 'canonical_low');
    assert.equal(successLog?.rawPayloadTraceable, true);
    assert.equal(successLog?.hasIngestionRunReference, true);
    assert.equal(successLog?.hasNormalizationFailures, false);
    assert.equal(successLog?.firstNormalizationFailureReason, null);
  } finally {
    await closeServer(server);
  }
});
