...
const { processMetaAdsSyncQueue } = await import(
	"../src/modules/meta-ads/index.js"
);
const { processGoogleAdsSyncQueue } = await import(
...
function buildMetaInsightFixture(
	...
	return {
		...
		...(level === "account"
			? {}
			: { campaign_id: "cmp_1", campaign_name: "Campaign One" }),
		...(level === "adset" || level === "ad"
			? { adset_id: "adset_1", adset_name: "Adset One" }
			: {}),
...
test(
	"Meta Ads and Google Ads sync preserve raw payloads without trimming",
	{ concurrency: false },
	async () => {
		...
		try {
			...
			globalThis.fetch = (async (input: string | URL | Request) => {
				...
				if (url.pathname.endsWith("/insights") && level) {
					const rawInsight = buildMetaInsightFixture(
						level as "account" | "campaign" | "adset" | "ad",
					);
					const malformedInsight =
						level === "ad" ? [buildMetaMalformedAdInsightFixture()] : [];
					return new Response(
						JSON.stringify({
			...
			assertExactRawPayloadInvariant({
				...
				unmappedPathValue: (
					(persisted.connection?.raw_account_data ?? {}) as {
						nested_debug: {
							arrays: Array<string | { beta: Array<string | null> }>;
						};
					}
			...
			assert.equal(
				persisted.connection?.raw_account_source,
				"meta_ads_account",
			);
			...
			const expectedAccountMetadata =
				__rawPayloadStorageTestUtils.buildRawPayloadStorageMetadata(
					expectedAccountInsight,
				);
			const expectedAdMetadata =
				__rawPayloadStorageTestUtils.buildRawPayloadStorageMetadata(
					expectedAdInsight,
				);
			const expectedMalformedAdMetadata =
				__rawPayloadStorageTestUtils.buildRawPayloadStorageMetadata(
			...
			assertExactRawPayloadInvariant({
				...
				unmappedPathValue: (
					persisted.spendRows[0].raw_payload as {
						source_debug: {
							nested: { kept: Array<string | null | { marker: string }> };
						};
					}
				).source_debug.nested.kept[2],
				expectedUnmappedPathValue:
					expectedAccountInsight.source_debug.nested.kept[2],
				normalizedVariant: {
					...expectedAccountInsight,
					objective: "outcome_traffic",
				},
			...
			assert.equal(
				persisted.spendRows[0].payload_size_bytes,
				expectedAccountMetadata.payloadSizeBytes,
			);
			assert.equal(
				persisted.spendRows[0].payload_hash,
				expectedAccountMetadata.payloadHash,
			);
			...
			assert.equal(
				persisted.spendRows[3].payload_size_bytes,
				expectedAdMetadata.payloadSizeBytes,
			);
			assert.equal(
				persisted.spendRows[3].payload_hash,
				expectedAdMetadata.payloadHash,
			);
			...
			assert.equal(
				persisted.spendRows[4].payload_size_bytes,
				expectedMalformedAdMetadata.payloadSizeBytes,
			);
			assert.equal(
				persisted.spendRows[4].payload_hash,
				expectedMalformedAdMetadata.payloadHash,
			);
							...
							JSON.stringify([
								{
									results: [
										buildGoogleCampaignFixture(),
										buildGoogleMalformedCampaignFixture(),
									],
								},
							]),
							{ status: 200, headers: { "content-type": "application/json" } },
			...
			assertExactRawPayloadInvariant({
				persistedRawPayload:
					googlePersisted.connection?.raw_customer_data ?? {},
				expectedRawPayload: expectedGoogleCustomer,
				unmappedPathValue: (
					(googlePersisted.connection?.raw_customer_data ?? {}) as {
						untouched_customer_debug: {
							nested: { values: Array<string | null | { deep: string }> };
						};
					}
				).untouched_customer_debug.nested.values[2],
				expectedUnmappedPathValue:
					expectedGoogleCustomer.untouched_customer_debug.nested.values[2],
				normalizedVariant: {
			...
			assert.equal(
				googlePersisted.connection?.raw_customer_source,
				"google_ads_customer",
			);
			assert.equal(
				googlePersisted.connection?.raw_customer_external_id,
				"1234567890",
			);
			...
			const expectedMalformedGoogleCampaign =
				buildGoogleMalformedCampaignFixture();
			const expectedMalformedGoogleAd = buildGoogleMalformedAdFixture();
			const expectedGoogleCampaignMetadata =
				__rawPayloadStorageTestUtils.buildRawPayloadStorageMetadata(
					expectedGoogleCampaign,
				);
			const expectedGoogleAdMetadata =
				__rawPayloadStorageTestUtils.buildRawPayloadStorageMetadata(
					expectedGoogleAd,
				);
			...
			const expectedMalformedGoogleAdMetadata =
				__rawPayloadStorageTestUtils.buildRawPayloadStorageMetadata(
			...
			assertExactRawPayloadInvariant({
				...
				expectedUnmappedPathValue:
					expectedGoogleCampaign.untouched_campaign_debug.labels,
				normalizedVariant: {
					...expectedGoogleCampaign,
					campaign: {
			...
			assert.equal(
				googlePersisted.spendRows[0].payload_source,
				"google_ads_api",
			);
			assert.equal(googlePersisted.spendRows[0].payload_external_id, "cmp_1");
			assert.equal(
				googlePersisted.spendRows[0].payload_size_bytes,
				expectedGoogleCampaignMetadata.payloadSizeBytes,
			);
			assert.equal(
				googlePersisted.spendRows[0].payload_hash,
				expectedGoogleCampaignMetadata.payloadHash,
			);
			assertExactRawPayloadInvariant({
				...
				expectedUnmappedPathValue:
					expectedMalformedGoogleCampaign.malformed_debug,
				normalizedVariant: {
					...expectedMalformedGoogleCampaign,
					campaign: {
			...
			assert.equal(
				googlePersisted.spendRows[1].payload_source,
				"google_ads_api",
			);
			assert.equal(googlePersisted.spendRows[1].payload_external_id, null);
			assert.equal(
				googlePersisted.spendRows[1].payload_size_bytes,
				expectedMalformedGoogleCampaignMetadata.payloadSizeBytes,
			);
			assert.equal(
				googlePersisted.spendRows[1].payload_hash,
				expectedMalformedGoogleCampaignMetadata.payloadHash,
			);
			assertExactRawPayloadInvariant({
				...
				expectedUnmappedPathValue:
					expectedGoogleAd.untouched_ad_debug.nested[2],
				normalizedVariant: {
					...expectedGoogleAd,
					adGroupAd: {
			...
			assert.equal(
				googlePersisted.spendRows[2].payload_source,
				"google_ads_api",
			);
			assert.equal(googlePersisted.spendRows[2].payload_external_id, "ad_1");
			assert.equal(
				googlePersisted.spendRows[2].payload_size_bytes,
				expectedGoogleAdMetadata.payloadSizeBytes,
			);
			assert.equal(
				googlePersisted.spendRows[2].payload_hash,
				expectedGoogleAdMetadata.payloadHash,
			);
			assertExactRawPayloadInvariant({
			...
			assert.equal(
				googlePersisted.spendRows[3].payload_source,
				"google_ads_api",
			);
			assert.equal(googlePersisted.spendRows[3].payload_external_id, null);
			assert.equal(
				googlePersisted.spendRows[3].payload_size_bytes,
				expectedMalformedGoogleAdMetadata.payloadSizeBytes,
			);
			assert.equal(
				googlePersisted.spendRows[3].payload_hash,
				expectedMalformedGoogleAdMetadata.payloadHash,
			);
		} finally {
