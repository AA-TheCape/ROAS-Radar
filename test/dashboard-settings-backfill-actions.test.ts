...
function createFetchStub(calls: FetchCall[]) {
	return async (input: string | URL | Request, init?: RequestInit) => {
		...
		if (
			url.pathname === "/api/admin/attribution/orders/backfill" &&
			method === "POST"
		) {
			return jsonResponse(
				{
...
function createSettingsHarness(
	SettingsAdminView: (props: Record<string, unknown>) => React.ReactNode,
	...
	return function SettingsHarness() {
		...
		return h(SettingsAdminView, {
			...
			onShopifyAttributionRecovery: async () => {
				const response = await api.recoverShopifyAttributionHints(
...
test("settings recovery harness preserves the existing recovery actions and submits dry-run payload defaults", async () => {
	...
	try {
		const [{ default: SettingsAdminView }, api] = await Promise.all([
			...
			loadDashboardModule<typeof import("../dashboard/src/lib/api")>(
		...
		try {
			...
			const recoverButton = await waitForButton(
				mounted.container,
				"Recover attribution hints",
			);
			click(recoverButton);
			...
			const recoveryCall = calls.find(
				(call) => call.path === "/api/admin/shopify/orders/recover-attribution",
			);
			...
			const selectButton = await waitForButton(
				mounted.container,
				"Select attribution backfill",
			);
			click(selectButton);
			...
			assert.match(
				optionsText,
				/Run a dry run first for this exact window, then keep the same dates/,
			);
			assert.match(
				optionsText,
			...
			const queueButton = await waitForButton(
				mounted.container,
