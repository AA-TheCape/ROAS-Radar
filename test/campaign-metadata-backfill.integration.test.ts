...
test('campaign metadata resolution keeps duplicate platform entity ids isolated by account', async () => {
  await resetE2EDatabase();
  ...
  try {
    ...
    await pool.query(
      `
        ...
        VALUES
          (1, 1, '2026-04-10'::date, 'campaign', 'east-row', 'acct-google-1', 'Google East', 'cmp_shared', 'Brand Search East Raw', 'google', 'cpc', 'brand-east', 'unknown', 'unknown', 'USD', '120.00', 0, 0, '{}'::jsonb),
          (2, 2, '2026-04-10'::date, 'campaign', 'west-row', 'acct-google-2', 'Brand Search West Raw', 'cmp_shared', 'Brand Search West Raw', 'google', 'cpc', 'brand-west', 'unknown', 'unknown', 'USD', '150.00', 0, 0, '{}'::jsonb)
      `
    );
    ...
    const result = await resolveCampaignDisplayMetadata('2026-04-10', '2026-04-10', ['brand-east', 'brand-west'], 'google');

    assert.deepEqual(result.byCampaign.get('brand-east'), {
      campaign: 'brand-east',
      source: 'google',
      medium: 'cpc',
      campaignDisplayName: 'Brand Search East Latest',
      campaignEntityId: 'cmp_shared',
    ...
    assert.deepEqual(result.byCampaign.get('brand-west'), {
      campaign: 'brand-west',
      source: 'google',
      medium: 'cpc',
      campaignDisplayName: 'Brand Search West Latest',
      campaignEntityId: 'cmp_shared',
      campaignPlatform: 'google_ads',
      campaignNameResolutionStatus: 'resolved',
