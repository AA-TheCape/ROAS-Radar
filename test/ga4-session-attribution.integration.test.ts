...
test("GA4 session attribution ingestion upserts normalized rows idempotently and advances the hourly watermark", async () => {
	...
	assert.deepEqual(firstRun.processedHours, [
	...
	assert.deepEqual(secondRun.processedHours, [
		"2026-04-27T10:00:00.000Z",
		"2026-04-27T11:00:00.000Z",
	]);
	...
	const counts = await pool.query<{ count: string }>(
		"SELECT COUNT(*)::text AS count FROM ga4_session_attribution",
	);
	...
	const [
		{
			ingestGa4SessionAttribution,
			getGa4SessionAttributionWatermark,
			markGa4SessionAttributionRunFailed,
		},
		{ pool },
	] = await Promise.all([
		import("../src/modules/attribution/ga4-session-attribution.js"),
