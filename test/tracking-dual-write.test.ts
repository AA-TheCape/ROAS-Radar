...
test("tracking endpoint dual-writes browser events into the attribution touch store", async () => {
	...
	pool.query = (async (text: string) => {
		...
		if (
			text.includes("FROM tracking_events") &&
			text.includes("WHERE ingestion_fingerprint = $1")
		) {
			return { rowCount: 0, rows: [] };
		}
	...
	try {
		...
		assert.deepEqual(
			JSON.parse(String(browserEventInsert.params?.[23])),
			validTrackPayload,
		);
...
test("tracking endpoint persists the full raw browser payload before schema filtering or normalization", async () => {
	...
	try {
		const { response, body } = await postTrack(
			server,
		...
		assert.equal(
			browserEventInsert.params?.[18],
			"checkout-token-with-whitespace",
		);
		assert.equal(
			browserEventInsert.params?.[19],
			"browser-event-with-whitespace",
		);
		assert.deepEqual(
			JSON.parse(String(browserEventInsert.params?.[23])),
			rawPayloadWithExtraFields,
...
test("tracking endpoint keeps the browser event when the derived server attribution leg fails", async () => {
	...
	pool.connect = (async () => {
		...
		return {
			query: buildClientQueryHandler([], connectCalls, {
				failServerAttribution: connectCalls === 2,
			}),
			release: () => undefined,
		};
	}) as typeof pool.connect;
...
test("tracking endpoint deduplicates the backend-derived attribution leg against an existing mirrored touch", async () => {
	...
	pool.connect = (async () => {
		...
		return {
			query: buildClientQueryHandler([], connectCalls, {
				deduplicateServerAttribution: connectCalls === 1,
			}),
			release: () => undefined,
		};
	}) as typeof pool.connect;
