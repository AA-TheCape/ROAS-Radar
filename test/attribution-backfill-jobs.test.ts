...
const backfillJobsModule = await import(
	"../src/modules/attribution/backfill-jobs.js"
);
const backfillModule = await import("../src/modules/attribution/backfill.js");

const { buildBackfillExecutionOptions, processOrderAttributionBackfillRuns } =
	backfillJobsModule;
...
test("buildBackfillExecutionOptions maps submitted options to backfill execution inputs", () => {
	...
	assert.equal(
		executionOptions.windowStart.toISOString(),
		"2026-04-01T00:00:00.000Z",
	);
	assert.equal(
		executionOptions.windowEnd.toISOString(),
		...
		processOrderAttributionBackfillRuns({
			...
			executeBackfillRun: async (options) => {
				...
				throw new OrderAttributionBackfillRunError(
					"Worker failed while writing report",
					{
