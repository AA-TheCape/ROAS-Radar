...
async function insertProtectedOrderLink(
	sessionId: string,
    ...
    `,
		[
			shopifyOrderId,
			sessionId,
			JSON.stringify({ id: shopifyOrderId, landing_session_id: sessionId }),
		],
	);
...
test.beforeEach(async () => {
	if (!pool || !runSessionAttributionRetention) {
		...
		runSessionAttributionRetention =
			retentionModule.runSessionAttributionRetention;
	}
...
test(
	...
	async () => {
		...
		assert.deepEqual(
			...
			[freshSession, protectedSession].sort(),
		);
		assert.deepEqual(
			await listTouchEventSessionIds(),
			[freshSession, protectedSession].sort(),
		);
	},
);
...
test(
	"runSessionAttributionRetention does not delete rows exactly on the retention cutoff",
	{ concurrency: false },
	async () => {
...
test(
	...
	async () => {
		...
		assert.deepEqual(await listGa4FallbackCandidateKeys(), [
			"cutoff-ga4-candidate",
			"fresh-ga4-candidate",
		]);
	},
);
