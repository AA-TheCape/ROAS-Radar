...
import { createDom, loadDashboardModule, tick } from './dashboard-ui-test-helpers';
...
test('React attribution tracker bootstraps the first landing and posts updated page views on SPA navigation', async () => {
  ...
  try {
    const trackingModule = await loadDashboardModule<typeof import('../dashboard/src/lib/attributionTracking')>(
    ...
    globalThis.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
      const url = new URL(typeof input === 'string' ? input : input instanceof Request ? input.url : input.toString(), dom.window.location.origin);
      ...
      if (url.pathname === '/track') {
        trackPayloads.push(JSON.parse(String(init?.body ?? '{}')) as Record<string, unknown>);
        return jsonResponse({
          ok: true,
          sessionId: '123e4567-e89b-42d3-a456-426614174000'
        });
...
test('React attribution tracker retries transient ingestion failures from durable storage', async () => {
  ...
  try {
    ...
    assert.equal(dom.window.localStorage.getItem('roas_radar_pending_track_events'), '[]');
  } finally {
    globalThis.fetch = previousFetch;
    dom.window.close();
  }
