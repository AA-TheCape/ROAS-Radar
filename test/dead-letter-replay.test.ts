...
test("failed Shopify writeback jobs retain payload and error context, and replay is auditable", async () => {
	...
	assert.equal(
		storedDeadLetter.rows[0].last_error_message,
		"shopify rejected note attributes",
	);
	...
	assert.equal(
		storedDeadLetter.rows[0].response_body,
		"invalid note attribute",
	);
	...
	assert.equal(
		deadLetterAfterReplay.rows[0].last_replay_run_id,
		String(replayResult.replayRunId),
	);
	...
	const replayRunItem = await modules.pool.query<{
		outcome: string;
		message: string | null;
	}>(
		`
...
test("replay can scope to a time window and requeue failed attribution jobs safely", async () => {
	...
	const insideAfterDryRun = await modules.pool.query<{
		status: string;
		dead_lettered_at: Date | null;
	}>(
		`
	...
	const dryRunItem = await modules.pool.query<{
		outcome: string;
		message: string | null;
	}>(
		`
	...
	assert.equal(
		dryRunItem.rows[0].message,
		"dry run only; source record was not requeued",
	);
	...
	const insideAfterReplay = await modules.pool.query<{
		status: string;
		dead_lettered_at: Date | null;
	}>(
		`
	...
	const outsideAfterReplay = await modules.pool.query<{
		status: string;
		dead_lettered_at: Date | null;
	}>(
		`
	...
	const outsideDeadLetter = await modules.pool.query<{
		status: string;
		last_replay_run_id: string | null;
	}>(
		`
