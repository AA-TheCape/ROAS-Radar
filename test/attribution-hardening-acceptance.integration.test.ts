...
let cachedModules:
  | {
      ...
      reconcileRecentShopifyOrderAttributes: typeof import('../src/modules/shopify/writeback.js').reconcileRecentShopifyOrderAttributes;
...
async function bootstrapSession(server: { address(): AddressInfo | null }) {
  ...
  return {
    landingUrl,
    referrerUrl,
    body: (await response.json()) as { sessionId: string; isNewSession: boolean }
  };
}
...
test('hard-to-lose acceptance preserves canonical capture, session id, and Shopify recovery when the browser event never arrives', async () => {
  ...
  try {
    ...
    await enqueueAttributionForOrder('hardening-order-1', 'acceptance_missing_browser_event');
    ...
    const attributeMap = new Map(appliedWritebacks[0].attributes.map((attribute) => [attribute.key, attribute.value]));
    assert.deepEqual(
      Object.fromEntries(attributeMap),
      {
        schema_version: '1',
        roas_radar_session_id: bootstrap.body.sessionId,
        landing_url: bootstrap.landingUrl,
