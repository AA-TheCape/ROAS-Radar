...
const snapshotFile = path.join(
	__dirname,
	"__snapshots__",
	"authenticated-ui-snapshots.json",
);

const React = requireFromDashboard("react") as {
	createElement: typeof import("react").createElement;
};
const { renderToStaticMarkup } = requireFromDashboard("react-dom/server") as {
						...
						topbarMeta: h(
							...
							h(
								...
								h(
									...
									{
										className:
											"font-semibold uppercase tracking-[0.14em] text-teal",
									},
									"Current time",
								),
...
test("authenticated route snapshots stay stable", async () => {
	...
	const expected = JSON.parse(readFileSync(snapshotFile, "utf8")) as Record<
		string,
		string
	>;
