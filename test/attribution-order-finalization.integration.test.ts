...
async function insertShopifyOrder(pool: Pool, input: ShopifyOrderInput): Promise<void> {
  const rawPayload = input.rawPayload ?? JSON.stringify({ id: input.shopifyOrderId, source_name: input.sourceName ?? 'web' });
  await pool.query(
    `
  ...
  await upsertGa4FallbackCandidates([
    {
      ...
      ga4UserKey: input.ga4ClientId ?? input.ga4SessionId ?? input.shopifyOrderId,
...
test('live GA4 rollout persists fallback attribution, snapshot provenance, and primary order credits end to end', async () => {
  ...
  assert.ok(primaryCredits.some((credit) => credit.attribution_model === 'last_touch'));
  assert.ok(primaryCredits.some((credit) => credit.attribution_model === 'first_touch'));
