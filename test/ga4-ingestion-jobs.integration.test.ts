...
async function getModules() {
	const [poolModule, jobsModule, sessionModule, deadLetterModule] =
		await Promise.all([
...
test("GA4 hourly job helpers enqueue explicit ranges once and claim them oldest first", async () => {
	...
	const hourStarts = listHourlyRange(
		"2026-04-27T08:13:00.000Z",
		"2026-04-27T10:59:00.000Z",
	);
...
test("GA4 hourly processing retries transient failures, then succeeds without duplicating persisted session rows", async () => {
	...
	const persistedRows = await listGa4SessionAttributionRows(
		(await getModules()).pool,
	);
	...
	assert.equal(
		await getGa4SessionAttributionWatermark((await getModules()).pool),
		targetHour,
	);
	assert.equal(await countPendingDeadLetters(), 0);
});
...
test("GA4 hourly processing dead-letters exhausted failures and replay requeues the hour for another attempt", async () => {
	...
	assert.deepEqual(
		(await listHourlyJobs()).map((job) => job.status),
		["completed"],
	);
});
