...
test("verifyWebhookSignature validates Shopify webhook signatures using the raw request body", () => {
	const rawBody = Buffer.from(
		JSON.stringify({ id: 123, source_name: "web" }),
		"utf8",
	);
	...
	assert.equal(
		__shopifyTestUtils.verifyWebhookSignature(rawBody, signature),
		true,
	);
	assert.equal(
		__shopifyTestUtils.verifyWebhookSignature(rawBody, `${signature}tampered`),
		false,
	);
});
...
test("buildLineItemExternalId reuses Shopify ids and falls back to a stable per-order key", () => {
	...
	assert.equal(
		__shopifyTestUtils.buildLineItemExternalId(
			"order-1",
			{ title: "Widget" },
			1,
		),
