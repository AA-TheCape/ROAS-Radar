...
async function getModules() {
  ...
  return {
    ...
    backfillRecentOrdersWithRecoveredAttribution: backfillModule.backfillRecentOrdersWithRecoveredAttribution,
    previewShopifyOrderWritebackAttributes: writebackModule.previewShopifyOrderWritebackAttributes,
...
test('production backfill recovers internal attribution and writes canonical Shopify attributes for recent recoverable orders', async () => {
  await resetIntegrationDatabase();
  const { pool, backfillRecentOrdersWithRecoveredAttribution, previewShopifyOrderWritebackAttributes } = await getModules();
  ...
  try {
    ...
    const capturedWritebacks: Array<{ shopifyOrderId: string; attributes: Array<{ key: string; value: string }> }> = [];
    ...
    const report = await backfillRecentOrdersWithRecoveredAttribution({
      ...
      applyWriteback: async (input) => {
        const attributes = await previewShopifyOrderWritebackAttributes(input.shopifyOrderId);
    ...
    const attributeMap = new Map(capturedWritebacks[0].attributes.map((attribute) => [attribute.key, attribute.value]));
