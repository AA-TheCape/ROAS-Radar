...
async function runPropagation(overrides = {}) {
	...
	const windowObject = {
		...
		crypto: {
			randomUUID:
				overrides.randomUUID || (() => "123e4567-e89b-42d3-a456-426614174000"),
		},
		fetch: wrappedFetch,
...
test("writes cart attributes when the first cart mutation request is observed", async () => {
	...
	assert.ok(
		syncCall,
		"expected cart attribute sync to run after first cart mutation",
	);
	assert.equal(syncCall.options.headers["X-ROAS-Radar-Cart-Sync"], "1");
	assert.equal(
		run.localStorageData.get("roas_radar_session_id"),
	...
	assert.deepEqual(body.attributes, {
		...
		landing_url:
			"https://store.example.com/products/widget?utm_source=google&utm_medium=cpc",
	...
	assert.equal(
		form.querySelector(
			'input[type="hidden"][name="attributes[schema_version]"]',
		).value,
		"1",
	);
	assert.equal(
		form.querySelector(
			'input[type="hidden"][name="attributes[roas_radar_session_id]"]',
		).value,
		"123e4567-e89b-42d3-a456-426614174111",
	);
});
...
test("retries transient Shopify failures with backoff and stable correlation IDs", async () => {
	...
	const syncCalls = run.fetchCalls.filter(
		(call) => call.url === "/cart/update.js",
	);
	...
	const retryLogs = run.consoleEntries.filter((entry) => {
		return (
			entry.level === "warn" &&
			entry.args[1] &&
			entry.args[1].event === "cart_attribute_sync_retry_scheduled"
		);
	});
...
test("reuses stored canonical attribution fields when later pages lose campaign params", async () => {
	...
	const syncCall = run.fetchCalls.find(
		(call) => call.url === "/cart/update.js",
	);
	const body = JSON.parse(syncCall.options.body);
	assert.equal(
