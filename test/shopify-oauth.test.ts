...
test("normalizeShopDomain accepts valid myshopify domains", () => {
	assert.equal(
		__shopifyTestUtils.normalizeShopDomain("Example-Shop.myshopify.com"),
...
test("verifyShopifyOAuthHmac validates Shopify callback signatures", () => {
	...
	const validHmac = createHmac("sha256", "test-secret")
		.update(message)
		.digest("hex");

	assert.equal(
		__shopifyTestUtils.verifyShopifyOAuthHmac(
			`${originalUrl}&hmac=${validHmac}`,
			validHmac,
		),
		true,
	);
...
test("buildShopifyInstallUrl includes expected OAuth parameters", () => {
	const installUrl = new URL(
		__shopifyTestUtils.buildShopifyInstallUrl(
			"example-shop.myshopify.com",
			"state-123",
			"/dashboard",
		),
	...
	assert.equal(
		...
		"https://api.example.com/shopify/oauth/callback",
	);
