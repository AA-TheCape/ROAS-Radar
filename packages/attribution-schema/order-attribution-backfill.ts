...
const isoTimestampSchema = z
	...
	.trim()
	.refine(
		(value) => ISO_TIMESTAMP_REGEX.test(value),
...
const dateOnlySchema = z
	.string()
	.trim()
	.regex(DATE_ONLY_REGEX, "Use YYYY-MM-DD.");

export const orderAttributionTierSchema = z.enum([
...
const organizationIdsSchema = z
	.array(
		z.coerce
			.number()
			.int()
			.positive("Organization ids must be positive integers."),
	)
	.max(
...
export const orderAttributionBackfillRequestSchema = z
	...
	.superRefine((value, ctx) => {
		if (
			...
			value.reclassificationTarget !== "meta_tier_reclassification"
		) {
			ctx.addIssue({
...
export const orderAttributionBackfillSubmittedOptionsSchema =
	orderAttributionBackfillRequestSchema;

export const orderAttributionBackfillJobStatusSchema = z.enum([
	"queued",
	"processing",
	"completed",
	"failed",
...
export type OrderAttributionBackfillRequest = z.infer<
	typeof orderAttributionBackfillRequestSchema
>;
export type OrderAttributionBackfillSubmittedOptions = z.infer<
	typeof orderAttributionBackfillSubmittedOptionsSchema
>;
export type OrderAttributionBackfillJobStatus = z.infer<
	typeof orderAttributionBackfillJobStatusSchema
>;
export type OrderAttributionBackfillFailure = z.infer<
	typeof orderAttributionBackfillFailureSchema
>;
export type OrderAttributionBackfillReport = z.infer<
	typeof orderAttributionBackfillReportSchema
>;
export type OrderAttributionBackfillEnqueueResponse = z.infer<
	typeof orderAttributionBackfillEnqueueResponseSchema
>;
export type OrderAttributionBackfillJobResponse = z.infer<
	typeof orderAttributionBackfillJobResponseSchema
>;

export function normalizeOrderAttributionBackfillRequest(
	input: unknown,
