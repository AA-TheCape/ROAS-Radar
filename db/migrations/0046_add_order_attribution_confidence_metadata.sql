...
ALTER TABLE shopify_orders
  ...
  ADD COLUMN IF NOT EXISTS attribution_confidence_score numeric(5, 4) NOT NULL DEFAULT 0,
...
WITH order_metadata AS (
  SELECT
    ...
    LEAST(GREATEST(COALESCE(results.confidence_score, 0), 0), 1)::numeric(5, 4) AS attribution_confidence_score,
  ...
  LEFT JOIN attribution_sources order_sources
    ON order_sources.code = COALESCE(orders.attribution_source, results.match_source)
  LEFT JOIN attribution_sources fallback_source
    ON fallback_source.code = 'unattributed'
  LEFT JOIN matching_methods methods
    ON methods.code = COALESCE(orders.attribution_reason, results.attribution_reason, results.match_source)
...
WITH result_metadata AS (
  ...
  LEFT JOIN attribution_sources fallback_source
    ON fallback_source.code = 'unattributed'
  LEFT JOIN matching_methods methods
    ON methods.code = COALESCE(results.attribution_reason, results.match_source)
  LEFT JOIN matching_methods fallback_method
    ON fallback_method.code = 'unknown'
)
