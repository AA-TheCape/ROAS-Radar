...
export function resolveOrderAttributionMaterializationExecution(now: Date): {
	...
	const lookbackDays =
		parseOptionalInteger("ORDER_ATTRIBUTION_MATERIALIZATION_LOOKBACK_DAYS") ??
		2;
	const lagDays =
		parseOptionalInteger("ORDER_ATTRIBUTION_MATERIALIZATION_LAG_DAYS") ?? 1;
	...
	const onlyWebOrders = parseBoolean(
