...
type IdentityGraphBackfillSource = (typeof IDENTITY_GRAPH_BACKFILL_SOURCES)[number];
...
export function resolveIdentityGraphBackfillExecution(now: Date): {
  ...
  const requestedBy =
    process.env.IDENTITY_GRAPH_BACKFILL_REQUESTED_BY?.trim() || 'cloud-run-scheduler';
  const workerId =
  ...
  const startAt =
    configuredStartAt ?? new Date(endAt.getTime() - lookbackDays * 24 * 60 * 60 * 1000);
