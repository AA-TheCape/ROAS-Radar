...
async function fetchJourneySummaryById(journeyId: string): Promise<IdentityJourneySummaryRow | null> {
...
async function fetchJourneySummaryByLookup(nodeType: (typeof identityNodeTypes)[number], nodeKey: string): Promise<IdentityJourneySummaryRow | null> {
  const result = await query<IdentityJourneySummaryRow>(
    `
      SELECT
...
async function fetchJourneyEdges(journeyId: string): Promise<IdentityEdgeRow[]> {
...
async function fetchJourneySessions(journeyId: string): Promise<JourneySessionRow[]> {
...
async function fetchJourneyOrders(journeyId: string): Promise<JourneyOrderRow[]> {
