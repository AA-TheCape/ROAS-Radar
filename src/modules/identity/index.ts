...
export function resolveIdentityStitch(
	...
	input: Pick<IdentityStitchInput, "shopifyCustomerId" | "email"> & {
		emailHash?: string | null;
	},
	...
	const identityByCustomerId = shopifyCustomerId
		? (existingIdentities.find(
				(identity) => identity.shopify_customer_id === shopifyCustomerId,
			) ?? null)
		: null;
	const identityByEmailHash = emailHash
		? (existingIdentities.find(
				(identity) => identity.hashed_email === emailHash,
			) ?? null)
		: null;
...
export async function ingestIdentityEdges(
	...
	if (normalizedNodes.length === 0) {
		...
		return {
			...
			deduplicated: false,
			emailHash:
				normalizedNodes.find((node) => node.nodeType === "hashed_email")
					?.nodeKey ?? null,
			linkedSessionIds: [],
			metrics,
		};
	...
	if (registeredIngestion.deduplicated) {
		const linkedSessionIds = registeredIngestion.existingJourneyId
			? await collectJourneySessionIds(
					client,
					registeredIngestion.existingJourneyId,
				)
			: [];
		const metrics = {
		...
		return {
			outcome: metrics.outcome,
			reason:
				metrics.outcome === "conflict" ? AUTHORITATIVE_CONFLICT : "linked",
			journeyId: registeredIngestion.existingJourneyId,
			deduplicated: true,
			emailHash:
				normalizedNodes.find((node) => node.nodeType === "hashed_email")
	...
	const nodeRows = await upsertAndLockIdentityNodes(
		client,
		normalizedNodes,
		sourceTimestamp,
	);
	const qualifyingIdentityObservedAt = hasQualifyingIdentityNodes(
		normalizedNodes,
	)
		? sourceTimestamp
		: null;
	const candidateRows = await loadActiveJourneyCandidates(
		client,
	...
	const journeyResolution = await resolveWinningJourney(client, {
		...
		incomingShopifyCustomerId:
			normalizedNodes.find((node) => node.nodeType === "shopify_customer_id")
				?.nodeKey ?? null,
		evidenceSource: input.evidenceSource,
		sourceTable: input.sourceTable,
		sourceRecordId: input.sourceRecordId,
	...
	for (const node of nodeRows) {
		...
		const activeCandidate = candidateRows.find(
			(row) => row.node_id === node.id && row.edge_id,
		);
		const desiredEdgeType =
			node.node_type === "shopify_customer_id"
				? "authoritative"
		...
		if (activeCandidate.journey_id === winnerJourneyId) {
			await touchActiveIdentityEdge(
				client,
				activeCandidate.edge_id,
				sourceTimestamp,
			);
			continue;
		}
		...
		await linkSupersededIdentityEdge(
			client,
			activeCandidate.edge_id,
			newEdgeId,
		);
	...
	await refreshJourneyConvenienceFields(
		client,
		winnerJourneyId,
		sourceTimestamp,
		{
			...
			emailHash:
				normalizedNodes.find((node) => node.nodeType === "hashed_email")
					?.nodeKey ?? null,
			phoneHash:
				normalizedNodes.find((node) => node.nodeType === "phone_hash")
					?.nodeKey ?? null,
			graphChanged:
				journeyResolution.created ||
				attachedNodes > 0 ||
				rehomedNodes > 0 ||
	...
	for (const row of candidateRows) {
		...
		await markJourneyMergedIfEmpty(
			client,
			row.journey_id,
			winnerJourneyId,
			sourceTimestamp,
	...
	const linkedSessionIds = await syncIdentityReferences(client, {
		journeyId: winnerJourneyId,
		sessionId:
			normalizedNodes.find((node) => node.nodeType === "session_id")?.nodeKey ??
			null,
		checkoutToken:
			normalizedNodes.find((node) => node.nodeType === "checkout_token")
				?.nodeKey ?? null,
		cartToken:
			normalizedNodes.find((node) => node.nodeType === "cart_token")?.nodeKey ??
			null,
		shopifyCustomerId:
			normalizedNodes.find((node) => node.nodeType === "shopify_customer_id")
				?.nodeKey ?? null,
		emailHash:
			normalizedNodes.find((node) => node.nodeType === "hashed_email")
				?.nodeKey ?? null,
		phoneHash:
			normalizedNodes.find((node) => node.nodeType === "phone_hash")?.nodeKey ??
			null,
		qualifyingIdentityObservedAt,
	});
...
async function registerIdentityEdgeIngestion(
	...
	if (!existingRun) {
		throw new Error(
			`identity edge ingestion run disappeared for idempotency key ${input.idempotencyKey}`,
		);
	}
...
function buildNormalizedIdentityNodes(
	input: IdentityIngestionInput,
): IdentityNodeInput[] {
...
async function loadActiveJourneyCandidates(
	client: DbClient,
	nodeIds: string[],
): Promise<IdentityJourneyCandidateRow[]> {
	if (nodeIds.length === 0) {
		return [];
	}
	...
	return result.rows.filter(
		(row) =>
			row.journey_id === null ||
			ACTIVE_IDENTITY_STATUSES.has(
				row.edge_type === "quarantined" ? "quarantined" : "active",
			),
	);
}

async function loadJourneyScores(
	client: DbClient,
	journeyIds: string[],
): Promise<Map<string, IdentityJourneyScoreRow>> {
...
> {
	...
	if (candidateJourneyIds.length === 0) {
		const journeyId = await createJourney(client, {
			...
			emailHash:
				input.nodeRows.find((row) => row.node_type === "hashed_email")
					?.node_key ?? null,
			phoneHash:
				input.nodeRows.find((row) => row.node_type === "phone_hash")
					?.node_key ?? null,
	...
	if (authoritativeIds.length > 1 && incomingAuthoritativeJourney) {
		for (const row of workingCandidates) {
			...
			if (
				row.authoritative_shopify_customer_id === null ||
				row.authoritative_shopify_customer_id ===
					input.incomingShopifyCustomerId
			) {
				continue;
		...
		workingCandidates = workingCandidates.filter(
			(row) => !input.quarantinedNodeIds.has(row.node_id),
		);
	...
	const fallbackJourneyId = await createJourney(client, {
		...
		emailHash:
			input.nodeRows.find((row) => row.node_type === "hashed_email")
				?.node_key ?? null,
		phoneHash:
			input.nodeRows.find((row) => row.node_type === "phone_hash")?.node_key ??
			null,
		sourceTimestamp: input.sourceTimestamp,
		lookbackAnchorTimestamp: input.sourceTimestamp,
		evidenceSource: input.evidenceSource,
...
function distinctAuthoritativeShopifyIds(
...
async function createJourney(
	...
	const lookbackWindow = buildHistoricalLookbackWindow(
		input.lookbackAnchorTimestamp,
	);
	await client.query(
		`
...
async function insertIdentityEdge(
	...
	if (!nodeType) {
		throw new Error(
			`Identity node ${input.nodeId} was not found while inserting edge`,
		);
	}
...
async function touchActiveIdentityEdge(
	client: DbClient,
	edgeId: string,
	sourceTimestamp: Date,
): Promise<void> {
...
async function deactivateIdentityEdge(
	client: DbClient,
	edgeId: string,
): Promise<void> {
	await client.query(
...
async function linkSupersededIdentityEdge(
	client: DbClient,
	previousEdgeId: string,
	supersedingEdgeId: string,
): Promise<void> {
	await client.query(
		`
      UPDATE identity_edges
...
async function upsertCompatibilityCustomerIdentity(
	...
	const safelyAssignableFields = await resolveCompatibilityIdentityAssignments(
		client,
		journeyId,
		{
...
async function resolveCompatibilityIdentityAssignments(
	...
	return {
		emailHash:
			emailOwnerId && emailOwnerId !== journeyId ? null : input.emailHash,
		shopifyCustomerId:
			customerOwnerId && customerOwnerId !== journeyId
				? null
				: input.shopifyCustomerId,
	};
}
...
async function syncIdentityReferences(
	...
	const compatibilityExists = await customerIdentityExists(
		client,
		input.journeyId,
	);
	...
	const lookbackWindow = input.qualifyingIdentityObservedAt
		...
		: null;
	const safeOrderRewriteContactHashes =
		await resolveSafeOrderRewriteContactHashes(client, input.journeyId, {
			emailHash: input.emailHash,
			phoneHash: input.phoneHash ?? null,
		});
...
async function resolveSafeOrderRewriteContactHashes(
	...
	const nodeKeys = nodeTypes.map(
		(nodeType) => hashesByType.get(nodeType) as string,
	);
	...
	const safeHashes = new Map(
		result.rows.map((row) => [row.node_type, row.node_key]),
	);
...
async function collectJourneySessionIds(
	client: DbClient,
	journeyId: string,
): Promise<string[]> {
...
async function loadJourneyLookbackWindow(
	client: DbClient,
	journeyId: string,
): Promise<IdentityJourneyLookbackWindow | null> {
	const result = await client.query<{
...
async function customerIdentityExists(
	client: DbClient,
	journeyId: string,
): Promise<boolean> {
...
function emitIdentityIngestionMetrics(input: {
	...
	logInfo(
		"identity_edge_ingestion_processed",
...
function buildHistoricalLookbackWindow(
	anchorTimestamp: Date,
): IdentityJourneyLookbackWindow {
	return {
		lookbackWindowStartedAt: new Date(
			anchorTimestamp.getTime() -
