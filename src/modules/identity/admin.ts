...
function withValidDateRange<TSchema extends z.ZodRawShape>(schema: z.ZodObject<TSchema>) {
...
async function fetchIdentityHealthOverview(input: { startDate: string; endDate: string; source?: string }) {
  ...
  const latestBackfillOptions =
    latestBackfill && typeof latestBackfill.options === 'object' && latestBackfill.options !== null
      ...
      latestRun: latestBackfill
        ? {
            ...
            sources: Array.isArray(latestBackfillOptions?.sources)
              ? latestBackfillOptions.sources.filter((value): value is string => typeof value === 'string')
              : [],
