...
type MetaAttributionEvidenceRow = {
  id: string;
  eligibility_outcome: MetaEligibilityOutcome;
  confidence_score: string | number | null;
  order_occurred_at_utc: Date | null;
  meta_touchpoint_occurred_at_utc: Date | null;
  match_basis: string | null;
  currency_code: string | null;
};

type MetaAttributionEvidencePayload = z.infer<typeof metaAttributionEvidenceSchema>;

type NormalizedMetaAttributionEvidence = {
  organizationId: number;
  shopifyOrderId: string;
  metaConnectionId: number | null;
  ...
  eventOrReportTimestampUtc: Date | null;
  reportedConversionTimestampUtc: Date | null;
  attributionWindowDays: number;
  metaAttributionReason: string;
  campaignId: string;
  ...
  reportedConversionValue: number | null;
  reportedEventName: string | null;
  isViewThrough: boolean;
  isClickThrough: boolean;
  matchBasis: ApprovedMatchBasis | null;
  observedMatchBases: ApprovedMatchBasis[];
...
function parseInput(input: unknown): MetaAttributionEvidencePayload {
  try {
    return metaAttributionEvidenceSchema.parse(input);
  } catch (error) {
    if (error instanceof z.ZodError) {
      throw new MetaAttributionEvidenceHttpError(
        400,
        'invalid_request',
        'Invalid Meta attribution evidence payload',
        error.flatten()
      );
    }
...
function parseRequiredTimestamp(value: string, field: string): Date {
...
function parseOptionalTimestamp(value: string | undefined, field: string): Date | null {
  if (!value) {
    return null;
  }
...
function normalizeMatchBasis(
  value: string | undefined,
  normalizationFailures: string[]
): ApprovedMatchBasis | null {
  ...
  if ((APPROVED_MATCH_BASES as readonly string[]).includes(normalized)) {
    return normalized as ApprovedMatchBasis;
  }
...
function normalizeObservedMatchBases(
  input: string[] | undefined,
  strongestBasis: ApprovedMatchBasis | null,
  normalizationFailures: string[]
): ApprovedMatchBasis[] {
  const deduped = new Set<ApprovedMatchBasis>();
...
function buildInlinePayloadHash(rawPayload: unknown): string | null {
...
function buildSourceSnapshot(payload: MetaAttributionEvidencePayload): Record<string, unknown> {
  return {
...
function normalizePayload(
  payload: MetaAttributionEvidencePayload,
  orderRow: OrderTimestampRow
): NormalizedMetaAttributionEvidence {
  ...
  const metaTouchpointOccurredAtUtc = parseOptionalTimestamp(
    payload.metaTouchpointOccurredAtUtc,
    'metaTouchpointOccurredAtUtc'
  );
  const eventOrReportTimestampUtc = parseOptionalTimestamp(
    payload.eventOrReportTimestampUtc,
    'eventOrReportTimestampUtc'
  );
  const reportedConversionTimestampUtc = parseOptionalTimestamp(
    payload.reportedConversionTimestampUtc,
  ...
  const rawPayloadReference =
    normalizeNullableString(payload.rawPayloadReference) ??
...
async function upsertMetaAttributionEvidence(
  normalized: NormalizedMetaAttributionEvidence
): Promise<MetaAttributionEvidenceRow> {
  return withTransaction(async (client) => {
    const result = await client.query<MetaAttributionEvidenceRow>(
      `
...
export function createMetaAttributionEvidenceAdminRouter(): Router {
  const router = Router();
  ...
  router.post('/attribution-evidence', async (req, res, next) => {
    try {
      ...
      logInfo('meta_order_attribution_evidence_ingested', {
        ...
        confidenceBucket: normalized.confidenceScore === null
          ? 'unknown'
          : normalized.confidenceScore < 0.35
            ? 'below_parallel_floor'
            : normalized.confidenceScore < 0.5
              ? 'parallel_band'
              : normalized.confidenceScore < 0.75
                ? 'canonical_low'
                : 'canonical_high',
        isClickThrough: normalized.isClickThrough,
        isViewThrough: normalized.isViewThrough,
        rawPayloadTraceable: Boolean(normalized.rawPayloadReference || normalized.rawRecordId !== null),
        hasIngestionRunReference: normalized.ingestionRunId !== null,
        hasNormalizationFailures: normalized.normalizationFailures.length > 0,
        firstNormalizationFailureReason: normalized.normalizationFailures[0] ?? null,
        firstDisqualificationReason: normalized.disqualificationReasons[0] ?? null,
        firstParallelOnlyReason: normalized.parallelOnlyReasons[0] ?? null,
      ...
      if (error instanceof MetaAttributionEvidenceHttpError && error.statusCode < 500) {
        logWarning('meta_order_attribution_evidence_rejected', {
          ...
          shopifyOrderId: normalizeNullableString((req.body as { shopifyOrderId?: string } | undefined)?.shopifyOrderId),
          metaSignalId: normalizeNullableString((req.body as { metaSignalId?: string } | undefined)?.metaSignalId),
