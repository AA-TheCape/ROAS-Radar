...
const legacyMetaAdsModule = (await import(
  pathToFileURL(resolve(process.cwd(), 'dist/modules/meta-ads/index.js')).href
)) as {
  createMetaAdsRouter?: () => unknown;
  processMetaAdsSyncQueue: (options?: Record<string, unknown>) => Promise<Record<string, unknown>>;
  __metaAdsTestUtils?: Record<string, unknown>;
};

export const createMetaAdsRouter =
  legacyMetaAdsModule.createMetaAdsRouter ??
  (() => {
...
export function buildPlanningDates(now = new Date(), lastSyncCompletedAt: Date | null = null): string[] {
...
export function buildIncrementalPlanningDates(
  now = new Date(),
...
export function rollupPersistableSpendRows<T extends {
  rawRecordId: number;
  normalizedRow: {
    granularity: string;
    entityKey: string;
  ...
  for (const row of rows) {
    ...
    existing.normalizedRow.spend = (
      Number.parseFloat(existing.normalizedRow.spend) + Number.parseFloat(row.normalizedRow.spend)
    ).toFixed(2);
...
async function loadActiveMetaAdsConnections(): Promise<MetaAdsConnection[]> {
  const result = await query<MetaAdsConnection>(
    `
      SELECT
        id,
        ad_account_id,
...
export function createMetaAdsApiErrorForTest(
  statusCode: number,
  message: string,
  details: MetaAdsApiErrorShape
): Error & { statusCode: number; details: MetaAdsApiErrorShape } {
...
export function isRetryableMetaAdsApiError(error: unknown): boolean {
  ...
  const details = error.details as MetaAdsApiErrorShape;
...
export function formatMetaAdsError(error: unknown): string {
  ...
  const details = ('details' in error ? error.details : null) as MetaAdsApiErrorShape | null;
  const code = details?.error?.code;
