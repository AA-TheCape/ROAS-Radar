...
type MetaAdsCreativeMap = Record<
	string,
	{ creativeId: string | null; creativeName: string | null }
>;
...
function normalizeMetaAdsScopes(
...
function normalizeMetaAdAccountId(value: string): string {
	...
	if (!/^\d+$/.test(accountId)) {
		throw new MetaAdsHttpError(
			...
			"META_ADS_AD_ACCOUNT_ID must be numeric or act_<id>",
		);
	}
...
async function getResolvedMetaAdsConfig(): Promise<MetaAdsResolvedConfig> {
	if (!env.META_ADS_ENCRYPTION_KEY) {
		throw new MetaAdsHttpError(
			...
			"Missing Meta Ads configuration: META_ADS_ENCRYPTION_KEY",
		);
	}
	...
	const appSecret =
		stored?.app_secret?.trim() || env.META_ADS_APP_SECRET.trim();
	const appBaseUrl = (
		stored?.app_base_url?.trim() || env.META_ADS_APP_BASE_URL.trim()
	).replace(/\/$/, "");
	const appScopes = (
		stored?.app_scopes?.length ? stored.app_scopes : env.META_ADS_APP_SCOPES
	)
		.map((entry) => entry.trim())
		.filter(Boolean);
	const adAccountId =
		stored?.ad_account_id?.trim() || env.META_ADS_AD_ACCOUNT_ID.trim();
	const missing = [
	...
	if (missing.length > 0) {
		throw new MetaAdsHttpError(
			500,
			"meta_ads_config_missing",
			`Missing Meta Ads configuration: ${missing.join(", ")}`,
		);
	}
...
async function upsertMetaAdsSettings(
	payload: z.infer<typeof metaAdsConfigUpdateSchema>,
): Promise<void> {
	if (!env.META_ADS_ENCRYPTION_KEY) {
		throw new MetaAdsHttpError(
			500,
	...
	const secretProvided =
		typeof payload.appSecret === "string" &&
...
function buildMetaAdsAuthorizationUrl(
...
function calculateTokenExpiresAt(
...
function buildPlanningDates(
	...
	const currentBusinessDate = parseDateOnly(
		formatDateInTimeZone(now, META_ADS_SYNC_TIME_ZONE),
	);
...
function buildInsightsEntityId(
...
function rollupNormalizedSpendRows(
	rows: MetaAdsNormalizedSpendRow[],
): MetaAdsNormalizedSpendRow[] {
...
function rollupPersistableSpendRows(
	rows: MetaAdsPersistableSpendRow[],
): MetaAdsPersistableSpendRow[] {
			...
			if (audit) {
				await recordAdSyncApiTransaction({
					...
					errorMessage: response.ok
						? null
						: ((json as MetaAdsApiErrorBody | null)?.error?.message ??
...
async function exchangeCodeForAccessToken(
	...
	const url = new URL(
		`${META_GRAPH_BASE_URL}/${env.META_ADS_API_VERSION}/oauth/access_token`,
	);
	url.searchParams.set("client_id", config.appId);
...
async function fetchMetaAdsAccount(
	accessToken: string,
	adAccountId: string,
): Promise<MetaAdsAccountResponse> {
	const url = new URL(
		`${META_GRAPH_BASE_URL}/${env.META_ADS_API_VERSION}/act_${adAccountId}`,
	);
...
async function consumeOAuthState(state: string): Promise<string | null> {
	...
	if (!result.rowCount) {
		throw new MetaAdsHttpError(
			...
			"The Meta Ads OAuth state is invalid or expired",
		);
	}
...
async function refreshMetaAdsConnectionToken(
	connection: MetaAdsConnectionRow,
): Promise<MetaAdsConnectionRow> {
	const config = await getResolvedMetaAdsConfig();
	const refreshed = await exchangeLongLivedAccessToken(
		config,
...
async function getUsableMetaAdsConnection(
	forceRefresh = false,
): Promise<MetaAdsConnectionRow> {
	...
	if (!connection) {
		throw new MetaAdsHttpError(
			...
			"No active Meta Ads connection was found",
		);
	}
...
async function planIncrementalSyncs(now = new Date()): Promise<number> {
	...
	for (const row of result.rows) {
		const dates = buildIncrementalPlanningDates(
			now,
			row.last_sync_completed_at,
			row.last_sync_planned_for,
		);
		...
		if (row.last_sync_planned_for !== today) {
			await query(
				"UPDATE meta_ads_connections SET last_sync_planned_for = $2::date, updated_at = now() WHERE id = $1",
				[row.id, today],
			);
		}
	}
...
async function fetchInsightsForLevel(
	...
	let nextUrl: URL | null = new URL(
		`${META_GRAPH_BASE_URL}/${env.META_ADS_API_VERSION}/act_${adAccountId}/insights`,
	);
...
async function fetchCreativeMap(
	...
	for (let index = 0; index < adIds.length; index += 50) {
		...
		const response = await metaFetchJson<
			Record<string, { creative?: { id?: string; name?: string } }>
		>(url, 2, {
			connectionId: audit.connectionId,
			syncJobId: audit.syncJobId,
...
async function persistDailySpendSnapshot(
	...
	for (const level of META_SPEND_LEVELS) {
        ...
        `,
			...
			const normalizedRows = normalizeInsightRows(
				row,
				params.creativeMap,
				params.currency,
			);
...
export async function processMetaAdsSyncQueue(
	options: MetaAdsQueueProcessOptions = {},
): Promise<MetaAdsQueueProcessResult> {
	const startedAt = Date.now();
	const workerId =
		options.workerId ?? `meta-ads-sync-${randomBytes(6).toString("hex")}`;
	...
	router.get("/oauth/callback", async (req, res, next) => {
		try {
			...
			if (!payload.code || !payload.state) {
				throw new MetaAdsHttpError(
					400,
					"meta_ads_oauth_invalid_callback",
			...
			const longLivedToken = await exchangeLongLivedAccessToken(
				config,
				shortLivedToken.access_token,
			);
			const adAccountId = normalizeMetaAdAccountId(config.adAccountId);
			const account = await fetchMetaAdsAccount(
			...
			await upsertMetaAdsConnection({
				...
				tokenType:
					longLivedToken.token_type ?? shortLivedToken.token_type ?? "Bearer",
