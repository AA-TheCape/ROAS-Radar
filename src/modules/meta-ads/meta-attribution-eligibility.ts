...
export function evaluateMetaAttributionEligibility(
  ...
  const metaTouchpointOccurredAtUtc = input.metaTouchpointOccurredAtUtc;
  const touchpointBeforeOrder =
    orderOccurredAtUtc !== null &&
    metaTouchpointOccurredAtUtc !== null &&
    metaTouchpointOccurredAtUtc.getTime() <= orderOccurredAtUtc.getTime();
  const withinAttributionWindow =
    orderOccurredAtUtc !== null &&
    metaTouchpointOccurredAtUtc !== null &&
    touchpointBeforeOrder &&
    metaTouchpointOccurredAtUtc.getTime() >=
      orderOccurredAtUtc.getTime() - input.attributionWindowDays * 24 * 60 * 60 * 1000;
