...
function assertOrderValueQueryDoesNotWriteSpendTables(sql: string): void {
  ...
  for (const table of forbiddenTables) {
    if (new RegExp(`\\b(insert\\s+into|update|delete\\s+from)\\s+${table}\\b`, 'i').test(normalizedSql)) {
      throw new Error(`Order-value persistence must not write ${table}`);
    }
  }
}
