...
type ShopifyWritebackProcessor = (payload: ShopifyWritebackPayload) => Promise<void>;
...
function normalizeOptionalClickId(value: string | null | undefined): string | null {
...
function extractRawPayloadAttributeMap(rawPayload: unknown): Map<string, string> {
  ...
  for (const key of ['note_attributes', 'attributes']) {
    ...
    for (const [attributeKey, attributeValue] of Object.entries(value as Record<string, unknown>)) {
...
function buildCanonicalAttributeRows(row: CanonicalAttributeSourceRow): CanonicalAttributeRow[] {
  ...
  for (const field of ATTRIBUTION_UTM_FIELDS) {
    const value = normalizeOptionalUtm(coalesce(row[field], row[`initial_${field}` as keyof CanonicalAttributeSourceRow] as string | null));
  ...
  for (const field of ATTRIBUTION_CLICK_ID_FIELDS) {
    const value = normalizeOptionalClickId(coalesce(row[field], row[`initial_${field}` as keyof CanonicalAttributeSourceRow] as string | null));
...
async function fetchCanonicalAttributeSource(db: Queryable, shopifyOrderId: string): Promise<CanonicalAttributeSourceRow | null> {
...
async function fetchCanonicalAttributeRows(db: Queryable, shopifyOrderId: string): Promise<CanonicalAttributeRow[] | null> {
...
async function defaultShopifyWritebackProcessor(payload: ShopifyWritebackPayload): Promise<void> {
...
let shopifyWritebackProcessor: ShopifyWritebackProcessor = defaultShopifyWritebackProcessor;
let enqueueHook: ((input: { shopifyOrderId: string; requestedReason: string }) => Promise<void>) | null = null;
...
async function claimShopifyOrderWritebackJobs(workerId: string, now: Date, limit: number): Promise<ShopifyOrderWritebackJob[]> {
...
export async function enqueueShopifyOrderWriteback(
  ...
  const result = await client.query<{ id: number; queue_key: string; status: string }>(
    `
...
async function markJobForRetry(job: ShopifyOrderWritebackJob, workerId: string, error: unknown): Promise<'retry' | 'dead_lettered'> {
...
export const __shopifyWritebackTestUtils = {
  ...
  setEnqueueHook(hook: ((input: { shopifyOrderId: string; requestedReason: string }) => Promise<void>) | null): void {
    enqueueHook = hook;
  }
};
