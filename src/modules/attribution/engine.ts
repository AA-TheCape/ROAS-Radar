...
export type AttributionModelOutputs = Record<
	AttributionModel,
	AttributionCredit[]
>;
...
const DEFAULT_RULE_BASED_WEIGHTS: Required<
	Pick<
		...
		| "middleTouchWeight"
		| "lastTouchWeight"
		| "clickIdBonusMultiplier"
		| "directDiscountMultiplier"
	>
> = {
...
function revenueToCents(value: number | string): number {
	...
	if (!Number.isFinite(numericValue) || numericValue < 0) {
		throw new Error(
			`orderRevenue must be a finite non-negative number, received ${String(value)}`,
		);
	}
...
function normalizeWeights(rawWeights: number[]): number[] {
	const positiveWeights = rawWeights.map((value) =>
...
function allocateRevenueAcrossWeights(
	totalCents: number,
	...
	let distributedCents = provisional.reduce(
		(sum, entry) => sum + entry.wholeCents,
		0,
	);
...
function buildCredits(
	...
	const creditedCents = allocateRevenueAcrossWeights(
		totalCents,
		normalizedWeights,
	);
	const highestCreditCents = creditedCents.reduce(
		(max, value) => Math.max(max, value),
		0,
	);
	const primaryTouchpointIndex = creditedCents.findIndex(
		(value) => value === highestCreditCents,
	);
...
export function computeSingleWinnerCredits(
	...
	const winnerWeights = touchpoints.map((_touchpoint, index) =>
		index === winnerIndex ? 1 : 0,
	);
	return buildCredits(
		attributionModel,
		touchpoints,
		normalizeWeights(winnerWeights),
		totalRevenue,
	);
}
...
function timeDecayWeights(
	...
	const halfLifeMs =
		toPositiveNumber(halfLifeDays, DEFAULT_TIME_DECAY_HALF_LIFE_DAYS) *
		24 *
		60 *
		60 *
		1000;
	...
	return touchpoints.map((touchpoint) => {
		const deltaMs = Math.max(
			orderOccurredAt.getTime() - touchpoint.occurredAt.getTime(),
			0,
		);
...
function positionBasedWeights(
	...
	const remainingWeight = Math.max(
		1 - normalizedFirstWeight - normalizedLastWeight,
		0,
	);
...
function ruleBasedWeights(
	touchpoints: AttributionTouchpoint[],
	config: RuleBasedWeightConfig | undefined,
): number[] {
	const mergedConfig = {
...
export function buildUnattributedTouchpoint(
	orderOccurredAt: Date,
...
export function computeAttributionOutputs(
	...
	const modelWeights: Record<AttributionModel, number[]> = {
		...
		position_based: normalizeWeights(
		...
		rule_based_weighted: normalizeWeights(
			ruleBasedWeights(touchpoints, options.ruleBasedWeightConfig),
		),
	};

	return ATTRIBUTION_MODELS.reduce<AttributionModelOutputs>(
		(outputs, attributionModel) => {
