...
import { assertGa4BigQueryIngestionConfig, type Ga4BigQueryIngestionConfig } from './ga4-bigquery-config.js';

const GA4_SESSION_ATTRIBUTION_PIPELINE = 'ga4_session_attribution';
const GA4_INGESTION_LAG_ALERT_THRESHOLD_HOURS = 2;
const ALLOWED_CLICK_ID_KEYS = ['gclid', 'dclid', 'gbraid', 'wbraid', 'fbclid', 'ttclid', 'msclkid'] as const;

type AllowedClickIdKey = (typeof ALLOWED_CLICK_ID_KEYS)[number];
...
type Ga4SessionAttributionRawEventParam = {
  key?: unknown;
  value?: {
...
type Ga4SessionAttributionRawRow = {
...
type Ga4SessionAttributionNormalizedRow = {
  ...
  content: string | null;
  term: string | null;
  clickIdType: AllowedClickIdKey | null;
...
type PersistedGa4SessionAttributionRow = {
  ...
  content: string | null;
  term: string | null;
  click_id_type: AllowedClickIdKey | null;
...
type PersistedGa4SessionAttributionRowResult = {
  ga4_session_key: string;
  ga4_user_key: string;
  ga4_client_id: string | null;
  ga4_session_id: string;
...
export type Ga4HourlyExtractionResult = {
  hourStart: string;
  rows: Ga4SessionAttributionNormalizedRow[];
};
...
function normalizeEnabledConfig(config: Ga4BigQueryIngestionConfig | undefined): Extract<Ga4BigQueryIngestionConfig, { enabled: true }> {
...
export function planGa4SessionAttributionHourlyWindows(input: {
  ...
  const startHour = input.watermarkHour
    ? addHours(toHourStart(input.watermarkHour), -(config.ga4.backfillHours - 1))
...
function extractEventParamValue(param: Ga4SessionAttributionRawEventParam['value']): string | null {
...
export function extractAllowedGa4ClickIdsFromEventParams(eventParams: unknown): Partial<Record<AllowedClickIdKey, string>> {
  if (!Array.isArray(eventParams)) {
    return {};
  }

  const extracted: Partial<Record<AllowedClickIdKey, string>> = {};
  ...
  for (const rawEntry of eventParams) {
    ...
    const entry = rawEntry as Ga4SessionAttributionRawEventParam;
    const normalizedKey = normalizeLowercaseString(entry.key);

    if (!normalizedKey || !ALLOWED_CLICK_ID_KEYS.includes(normalizedKey as AllowedClickIdKey)) {
      continue;
    }

    const clickKey = normalizedKey as AllowedClickIdKey;
    if (extracted[clickKey]) {
      continue;
    }

    const normalizedValue = normalizeClickIdValue(normalizeNullableString(extractEventParamValue(entry.value)));
    if (!normalizedValue) {
      continue;
    }
...
function pickClickId(rawRow: Ga4SessionAttributionRawRow): {
  clickIdType: AllowedClickIdKey | null;
  clickIdValue: string | null;
} {
  ...
  if (explicitType && explicitValue && ALLOWED_CLICK_ID_KEYS.includes(explicitType as AllowedClickIdKey)) {
    return {
      clickIdType: explicitType as AllowedClickIdKey,
  ...
  const fromEventParams = extractAllowedGa4ClickIdsFromEventParams(rawRow.event_params);
  for (const key of ALLOWED_CLICK_ID_KEYS) {
  ...
  return {
    clickIdType: explicitType && ALLOWED_CLICK_ID_KEYS.includes(explicitType as AllowedClickIdKey)
      ? (explicitType as AllowedClickIdKey)
      : null,
    clickIdValue: explicitValue
  };
}

function normalizeMetadataSource(value: unknown): 'ga4_raw' | 'google_ads_transfer' | 'unresolved' {
...
function normalizeRawExtractionRow(raw: unknown): Ga4SessionAttributionNormalizedRow {
  if (!raw || typeof raw !== 'object') {
    throw new Error('GA4 extraction row must be an object');
  }

  const row = raw as Ga4SessionAttributionRawRow;
...
function mapNormalizedRowForPersistence(row: Ga4SessionAttributionNormalizedRow): PersistedGa4SessionAttributionRow {
  return {
...
async function readWatermarkHour(): Promise<Date | null> {
  const result = await query<{ watermark_hour: Date | null }>(
    `
...
async function markRunCompleted(client: PoolClient, completedAt: Date, watermarkHour: string | null): Promise<void> {
  await client.query(
    `
      INSERT INTO ga4_bigquery_ingestion_state (
        pipeline_name,
...
export async function markGa4SessionAttributionRunFailed(error: unknown): Promise<void> {
  await query(
    `
      INSERT INTO ga4_bigquery_ingestion_state (
        pipeline_name,
...
function mapPersistedRowForRead(row: PersistedGa4SessionAttributionRowResult): Ga4SessionAttributionNormalizedRow {
  return {
...
export async function listGa4SessionAttributionRows(db: Queryable): Promise<Ga4SessionAttributionNormalizedRow[]> {
  const result = await db.query<PersistedGa4SessionAttributionRowResult>(
    `
      SELECT
        ga4_session_key,
        ga4_user_key,
...
export async function getGa4SessionAttributionWatermark(db: Queryable): Promise<string | null> {
  const result = await db.query<{ watermark_hour: Date | null }>(
    `
      SELECT watermark_hour
      FROM ga4_bigquery_ingestion_state
      WHERE pipeline_name = $1
