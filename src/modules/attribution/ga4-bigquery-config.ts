...
export type Ga4BigQueryIngestionConfig =
  | {
      enabled: false;
    }
    | {
...
function validatePattern(value: string, pattern: RegExp, name: string): string {
...
function validatePositiveInteger(value: number | undefined, name: string): number {
...
function parseCustomerIds(value: string | undefined): string[] {
  const normalized = value?.trim() ?? '';
...
export function resolveGa4BigQueryIngestionConfig(
  ...
  const ga4ProjectId = validatePattern(
    normalizeRequiredString(source.GA4_BIGQUERY_PROJECT_ID, 'GA4_BIGQUERY_PROJECT_ID'),
    PROJECT_ID_PATTERN,
    'GA4_BIGQUERY_PROJECT_ID'
  );
  const ga4Location = validatePattern(
    normalizeRequiredString(source.GA4_BIGQUERY_LOCATION, 'GA4_BIGQUERY_LOCATION'),
    LOCATION_PATTERN,
    'GA4_BIGQUERY_LOCATION'
  );
  const ga4Dataset = validatePattern(
    normalizeRequiredString(source.GA4_BIGQUERY_DATASET, 'GA4_BIGQUERY_DATASET'),
    DATASET_PATTERN,
    'GA4_BIGQUERY_DATASET'
  );
  const ga4EventsTablePattern = validatePattern(
    normalizeRequiredString(source.GA4_BIGQUERY_EVENTS_TABLE_PATTERN, 'GA4_BIGQUERY_EVENTS_TABLE_PATTERN'),
    TABLE_PATTERN,
    'GA4_BIGQUERY_EVENTS_TABLE_PATTERN'
  );
  const ga4IntradayTablePattern = validatePattern(
    normalizeRequiredString(source.GA4_BIGQUERY_INTRADAY_TABLE_PATTERN, 'GA4_BIGQUERY_INTRADAY_TABLE_PATTERN'),
    TABLE_PATTERN,
    'GA4_BIGQUERY_INTRADAY_TABLE_PATTERN'
  );
  const ga4LookbackHours = parsePositiveInteger(
    source.GA4_BIGQUERY_LOOKBACK_HOURS,
  ...
  const adsProjectId = validatePattern(
    normalizeRequiredString(
      source.GOOGLE_ADS_TRANSFER_BIGQUERY_PROJECT_ID,
      'GOOGLE_ADS_TRANSFER_BIGQUERY_PROJECT_ID'
    ),
    PROJECT_ID_PATTERN,
    'GOOGLE_ADS_TRANSFER_BIGQUERY_PROJECT_ID'
  );
  const adsLocation = validatePattern(
    normalizeRequiredString(source.GOOGLE_ADS_TRANSFER_BIGQUERY_LOCATION, 'GOOGLE_ADS_TRANSFER_BIGQUERY_LOCATION'),
    LOCATION_PATTERN,
    'GOOGLE_ADS_TRANSFER_BIGQUERY_LOCATION'
  );
  const adsDataset = validatePattern(
    normalizeRequiredString(source.GOOGLE_ADS_TRANSFER_DATASET, 'GOOGLE_ADS_TRANSFER_DATASET'),
    DATASET_PATTERN,
    'GOOGLE_ADS_TRANSFER_DATASET'
  );
  const adsTablePattern = validatePattern(
    normalizeRequiredString(source.GOOGLE_ADS_TRANSFER_TABLE_PATTERN, 'GOOGLE_ADS_TRANSFER_TABLE_PATTERN'),
    TABLE_PATTERN,
    'GOOGLE_ADS_TRANSFER_TABLE_PATTERN'
  );
  const adsLookbackDays = parsePositiveInteger(
    source.GOOGLE_ADS_TRANSFER_LOOKBACK_DAYS,
    env.GOOGLE_ADS_TRANSFER_LOOKBACK_DAYS,
...
export function assertGa4BigQueryIngestionConfig(): Ga4BigQueryIngestionConfig {
