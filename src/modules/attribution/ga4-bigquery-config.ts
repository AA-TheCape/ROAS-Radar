...
function normalizeRequiredString(
	...
	if (!normalized) {
		throw new Error(
			`${name} is required when GA4 BigQuery ingestion is enabled`,
		);
	}
...
function parseCustomerIds(value: string | undefined): string[] {
	...
	return normalized.split(",").map((entry) => {
		...
		if (!CUSTOMER_ID_PATTERN.test(digitsOnly)) {
			throw new Error(
				`Invalid GA4_LINKED_GOOGLE_ADS_CUSTOMER_IDS entry: ${entry.trim()}`,
			);
		}
...
export function resolveGa4BigQueryIngestionConfig(
	...
	const enabled =
		parseBoolean(source.GA4_BIGQUERY_ENABLED) || env.GA4_BIGQUERY_ENABLED;
	...
	const ga4ProjectId = validatePattern(
		normalizeRequiredString(
			source.GA4_BIGQUERY_PROJECT_ID,
	...
	const ga4EventsTablePattern = validatePattern(
		normalizeRequiredString(
			source.GA4_BIGQUERY_EVENTS_TABLE_PATTERN,
	...
	const ga4IntradayTablePattern = validatePattern(
		normalizeRequiredString(
			source.GA4_BIGQUERY_INTRADAY_TABLE_PATTERN,
	...
	const adsLocation = validatePattern(
		normalizeRequiredString(
			source.GOOGLE_ADS_TRANSFER_BIGQUERY_LOCATION,
	...
	const adsTablePattern = validatePattern(
		normalizeRequiredString(
			source.GOOGLE_ADS_TRANSFER_TABLE_PATTERN,
