...
function normalizeAttributeArray(input: unknown): ShopifyAttributeLike[] {
  ...
  return input.filter((entry) => typeof entry === 'object' && entry !== null) as ShopifyAttributeLike[];
}

function getAttributeValue(attributes: ShopifyAttributeLike[], key: string): string | null {
...
function getAttributeValueFromKeys(attributes: ShopifyAttributeLike[], keys: string[]): string | null {
...
export function extractShopifyHintAttribution(payload: unknown): ShopifyHintAttribution | null {
