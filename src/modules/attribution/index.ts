...
type SyntheticAttributionInput = {
  ...
  matchSource: Extract<AttributionMatchSource, 'shopify_hint_fallback' | 'ga4_fallback' | 'unattributed'>;
...
export function buildProcessingMetricsLog(result: QueueRunSummary): string {
...
async function shouldPersistSyntheticAttribution(
  ...
  const result = await client.query<{ match_source: AttributionMatchSource; confidence_score: string }>(
    `
...
async function persistAttribution(
  ...
  order: OrderRow,
  journey: ResolvedJourney,
  options: SyntheticPersistOptions = {}
): Promise<void> {
  const orderOccurredAt = resolveOrderOccurredAt(order);
...
export async function processAttributionQueue(options: ProcessAttributionQueueOptions): Promise<QueueRunSummary> {
