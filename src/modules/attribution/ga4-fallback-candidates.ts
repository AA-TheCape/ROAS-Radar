...
function buildRetentionTimestamp(occurredAtIso: string): string {
	...
	retainedUntil.setUTCDate(
		retainedUntil.getUTCDate() +
...
function mapPersistedCandidate(
	row: PersistedGa4FallbackCandidateRow,
...
async function ensureTouchedPartitions(
	db: QueryExecutor,
	...
	for (const month of months) {
		await executeQuery(
			db,
			"SELECT ensure_ga4_fallback_candidate_partition($1::date)",
			[month],
		);
	}
}
...
function buildShouldReplaceGa4BundleSql(
	currentAlias: string,
...
>(client: QueryExecutor, text: string, params?: unknown[]) {
	if (client) {
...
export async function upsertGa4FallbackCandidates(
	...
	let upsertedRows = 0;
	const shouldReplaceBundleSql = buildShouldReplaceGa4BundleSql(
	...
	for (const candidate of candidates) {
		...
		if (
			...
			throw new Error(
				"GA4 fallback candidate requires at least one lookup key",
			);
		}
...
function countPopulatedDimensions(
	row: Pick<LookupRow, "source" | "medium" | "campaign" | "content" | "term">,
): number {
	return [row.source, row.medium, row.campaign, row.content, row.term].filter(
		Boolean,
	).length;
}

function compareSourceTableType(
	left: "events" | "intraday",
	right: "events" | "intraday",
): number {
	const precedence = {
		events: 0,
		intraday: 1,
...
function compareLookupRows(left: LookupRow, right: LookupRow): number {
	...
	const clickComparison =
		Number(Boolean(right.click_id_value)) -
	...
	const exportFreshnessComparison =
		right.source_export_hour.getTime() - left.source_export_hour.getTime();
	if (exportFreshnessComparison !== 0) {
		return exportFreshnessComparison;
	}

	const sourceTableTypeComparison = compareSourceTableType(
		left.source_table_type,
		right.source_table_type,
	);
	...
	const dimensionComparison =
		countPopulatedDimensions(right) - countPopulatedDimensions(left);
	if (dimensionComparison !== 0) {
		return dimensionComparison;
	}

	const sessionComparison = compareLexical(
		left.ga4_session_id,
		right.ga4_session_id,
	);
	if (sessionComparison !== 0) {
		return sessionComparison;
	}

	const clientComparison = compareLexical(
		left.ga4_client_id,
		right.ga4_client_id,
	);
	...
	const transactionComparison = compareLexical(
		left.transaction_id,
...
export async function listGa4FallbackCandidates(
	client?: PoolClient,
): Promise<PersistedGa4FallbackCandidate[]> {
	const db: QueryExecutor = client;
