...
export type Ga4FallbackRolloutMode =
	(typeof GA4_FALLBACK_ROLLOUT_MODES)[number];
...
function summarizeTouchpoint(touchpoint: ResolvedAttributionTouchpoint | null) {
	return {
		...
		confidenceScore: touchpoint
			? confidenceScoreFromJourneyWinner(
...
export function getGa4FallbackShadowThresholds() {
	const minEvaluatedOrders =
		Math.max(
			1,
			Math.trunc(
				Number(process.env.GA4_FALLBACK_SHADOW_MIN_EVALUATED_ORDERS ?? "100"),
			),
		) || 100;
	const maxAttributedOrderDeltaRate =
		Number(
			process.env.GA4_FALLBACK_SHADOW_MAX_ATTRIBUTED_ORDER_DELTA_RATE ?? "0.02",
		) || 0.02;
...
export async function persistGa4FallbackShadowComparison(
	...
	const shadowWouldChangeWinner =
		currentWinner.matchSource !== shadowWinner.matchSource;
...
export async function fetchGa4FallbackShadowReport(
	...
	const evaluatedOrders =
		Number.parseInt(row?.evaluated_orders ?? "0", 10) || 0;
	const shadowGa4FallbackOrders =
		Number.parseInt(row?.shadow_ga4_fallback_orders ?? "0", 10) || 0;
	const changedOrders = Number.parseInt(row?.changed_orders ?? "0", 10) || 0;
	const currentAttributedOrders =
		Number.parseInt(row?.current_attributed_orders ?? "0", 10) || 0;
	const shadowAttributedOrders =
		Number.parseInt(row?.shadow_attributed_orders ?? "0", 10) || 0;
	const currentAttributedRevenue = roundMetric(
		Number.parseFloat(row?.current_attributed_revenue ?? "0") || 0,
	);
	const shadowAttributedRevenue = roundMetric(
		Number.parseFloat(row?.shadow_attributed_revenue ?? "0") || 0,
	);
	const attributedOrderDelta = shadowAttributedOrders - currentAttributedOrders;
	const attributedRevenueDelta = roundMetric(
		shadowAttributedRevenue - currentAttributedRevenue,
	);
