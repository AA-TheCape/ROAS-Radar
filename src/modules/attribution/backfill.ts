...
export type OrderAttributionBackfillOptions = {
  ...
  onProgress?: (progress: OrderAttributionBackfillProgress) => Promise<void>;
};
...
function buildCreditMatchSource(journey: ResolvedJourney): string {
  return normalizeNullableString(journey.winner?.ingestionSource) ?? journey.tier;
}

function buildConfidenceLabel(confidenceScore: number): string {
...
async function persistAttribution(client: PoolClient, order: OrderRow, journey: ResolvedJourney): Promise<void> {
  ...
  const creditMatchSource = buildCreditMatchSource(journey);
  const confidenceLabel = buildConfidenceLabel(journey.confidenceScore);
...
export async function backfillRecentOrdersWithRecoveredAttribution(
  ...
  const failures: OrderAttributionBackfillFailure[] = [...progress.failures];
  const preview: BackfillPreviewRow[] = [...progress.preview];
