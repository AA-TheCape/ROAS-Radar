...
type ProcessOrderAttributionBackfillRunsOptions = {
	...
	claimRuns?: (
		...
		now: Date,
		limit: number,
	) => Promise<Array<OrderAttributionBackfillClaimedRun | LegacyClaimedRun>>;
	markRunCompleted?: (
		runId: string,
...
export function buildBackfillExecutionOptions(
	run:
		| Pick<OrderAttributionBackfillClaimedRun, "submittedBy" | "options">
		| Pick<LegacyClaimedRun, "submitted_by" | "options">,
	workerId: string,
...
export async function processOrderAttributionBackfillRuns(
	...
	const limit = Math.max(
		1,
		options.limit ?? DEFAULT_ORDER_ATTRIBUTION_BACKFILL_RUN_BATCH_SIZE,
	);
	const executeBackfillRun =
		options.executeBackfillRun ?? backfillRecentOrdersWithRecoveredAttribution;
	const claimRuns = options.claimRuns ?? claimOrderAttributionBackfillRuns;
	const markRunCompleted =
		options.markRunCompleted ?? markOrderAttributionBackfillRunCompleted;
	const markRunFailed =
		options.markRunFailed ?? markOrderAttributionBackfillRunFailed;
	...
	for (const run of runs) {
		...
		try {
			...
			const finalReport = orderAttributionBackfillReportSchema.parse(
				toOrderAttributionBackfillJobReport(detailedReport),
			);
