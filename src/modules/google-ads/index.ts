...
type GoogleAdsSpendGranularity =
	(typeof GOOGLE_ADS_SPEND_GRANULARITIES)[number];
...
type GoogleAdsMetadataConnection = Pick<
	GoogleAdsConnectionSecretRow,
	| "id"
	| "customer_id"
	| "login_customer_id"
	| "developer_token"
...
function extractGoogleAdsProviderRetryDelaySeconds(
	details: unknown,
): number | null {
...
function assertGoogleAdsConfig(): void {
	if (!env.GOOGLE_ADS_ENCRYPTION_KEY) {
		throw new GoogleAdsHttpError(
			...
			"Missing Google Ads configuration: GOOGLE_ADS_ENCRYPTION_KEY",
		);
	}
}

function normalizeGoogleAdsScopes(
...
async function getStoredGoogleAdsSettings(): Promise<GoogleAdsSettingsRow | null> {
	const result = await query<GoogleAdsSettingsRow>(
		`
      SELECT
        id,
        client_id,
...
async function getResolvedGoogleAdsConfig(): Promise<GoogleAdsResolvedConfig> {
	...
	const clientSecret =
		stored?.client_secret?.trim() || env.GOOGLE_ADS_CLIENT_SECRET.trim();
	const developerToken =
		stored?.developer_token?.trim() || env.GOOGLE_ADS_DEVELOPER_TOKEN.trim();
	const appBaseUrl = (
		stored?.app_base_url?.trim() || env.GOOGLE_ADS_APP_BASE_URL.trim()
	).replace(/\/$/, "");
	const appScopes = (
		stored?.app_scopes?.length ? stored.app_scopes : env.GOOGLE_ADS_APP_SCOPES
	)
		.map((entry) => entry.trim())
		.filter(Boolean);
	const missing = [
...
async function getGoogleAdsConfigurationSummary(): Promise<{
	source: "database" | "environment";
	clientId: string;
	appBaseUrl: string;
	appScopes: string[];
...
async function upsertGoogleAdsSettings(
	payload: z.infer<typeof googleAdsConfigUpdateSchema>,
): Promise<void> {
	assertGoogleAdsConfig();

	const clientSecretProvided =
		typeof payload.clientSecret === "string" &&
		payload.clientSecret.trim().length > 0;
	const developerTokenProvided =
		typeof payload.developerToken === "string" &&
...
function getGoogleAdsAppBaseUrl(config: GoogleAdsResolvedConfig): string {
	return new URL(config.appBaseUrl).toString().replace(/\/$/, "");
}

function buildGoogleAdsRedirectUri(config: GoogleAdsResolvedConfig): string {
...
function buildGoogleAdsAuthorizationUrl(
...
async function consumeOAuthState(
	state: string,
): Promise<GoogleAdsOAuthStateRow> {
	const result = await query<GoogleAdsOAuthStateRow>(
		`
	...
	if (!result.rowCount) {
		throw new GoogleAdsHttpError(
			400,
			"invalid_google_ads_oauth_state",
			"The Google Ads OAuth state is invalid or expired",
		);
	}
...
function buildAuthorizationCodeRequestBody(
	config: GoogleAdsResolvedConfig,
	code: string,
): URLSearchParams {
	const body = new URLSearchParams();
...
async function exchangeAuthorizationCode(
	config: GoogleAdsResolvedConfig,
	code: string,
): Promise<GoogleAdsAccessTokenResponse> {
	...
	const payload =
		(parseJsonResponsePayload(text) as GoogleAdsAccessTokenResponse) ?? {};
...
async function exchangeRefreshToken(
	...
	await query(
		"UPDATE google_ads_connections SET last_refreshed_at = now(), updated_at = now() WHERE id = $1",
		[connection.id],
	);
...
async function googleAdsSearch<T>(params: {
	...
	accessToken: string;
	gaql: string;
	audit?: GoogleAdsSyncAuditContext;
}): Promise<T[]> {
	const url = new URL(
	...
	const text = await response.text();
	const payload = parseJsonResponsePayload(text) as
		| Array<{ results?: T[] }>
...
async function fetchCustomerMetadata(
	...
	if (!row?.id) {
		throw new GoogleAdsHttpError(
			...
			"Unable to resolve the Google Ads customer",
		);
	}
...
function normalizeSpendSnapshot(params: {
	...
	const campaignSourceRows =
		params.campaignRows.length > 0 ? params.campaignRows : params.adRows;
	const accountSpend = campaignSourceRows.reduce(
		(total, row) =>
			total + Number(parseMicrosToDecimal(row.metrics?.costMicros)),
		0,
	);
	const accountImpressions = campaignSourceRows.reduce(
		(total, row) => total + parseMetricInteger(row.metrics?.impressions),
		0,
	);
	const accountClicks = campaignSourceRows.reduce(
		(total, row) => total + parseMetricInteger(row.metrics?.clicks),
		0,
	);
	...
	for (const row of params.campaignRows) {
		...
		upsertRow({
			...
			accountId: params.customer.customerId,
			accountName:
				row.customer?.descriptiveName ?? params.customer.descriptiveName,
	...
	for (const row of params.adRows) {
		...
		const adName =
			row.adGroupAd?.ad?.name ?? row.adGroupAd?.ad?.resourceName ?? null;
...
async function upsertGoogleAdsConnection(params: {
	...
	const rawPayloadMetadata = buildRawPayloadStorageMetadata(
		params.customer.rawPayload,
	);
...
async function getGoogleAdsConnectionById(
	connectionId: number,
): Promise<GoogleAdsConnectionSecretRow> {
	...
	if (!connection) {
		throw new GoogleAdsHttpError(
			...
			"No active Google Ads connection was found",
		);
	}
...
async function enqueueSyncDates(
	connectionId: number,
	dates: string[],
): Promise<number> {
	let enqueuedJobs = 0;
...
async function runReconciliation(params: {
	...
	const window = buildReconciliationWindow(
		params.now ?? new Date(),
		params.lastSyncCompletedAt ?? null,
	);
...
async function persistDailySpendSnapshot(
	...
	await client.query(
		"DELETE FROM google_ads_daily_spend WHERE connection_id = $1 AND report_date = $2::date",
		[params.connectionId, params.syncDate],
	);
	await client.query(
		"DELETE FROM google_ads_raw_spend_records WHERE connection_id = $1 AND report_date = $2::date",
		[params.connectionId, params.syncDate],
	);
	...
	for (const row of params.campaignRows) {
		...
		const { rawPayloadJson, payloadSizeBytes, payloadHash } =
			rawPayloadMetadata;
		...
		const insertResult = await client.query<
			{ id: number } & RawPayloadIntegrityRow
		>(
			`
...
async function markSyncJobFailed(
	...
	const retryDelaySeconds =
		providerRetryDelaySeconds ?? computeRetryDelaySeconds(job.attempts);
	const hasProviderRetryDelay = providerRetryDelaySeconds !== null;
	const shouldRetry =
		hasProviderRetryDelay || job.attempts < env.GOOGLE_ADS_SYNC_MAX_RETRIES;
...
async function processSyncJob(
	job: GoogleAdsSyncJobRow,
): Promise<GoogleAdsSyncJobOutcome> {
	await query(
		`
      UPDATE google_ads_connections
      SET
	...
	const attemptJob = async (): Promise<void> => {
		...
		const campaignRows = await fetchCampaignSpendRows(
			connection,
			accessToken,
			job.sync_date,
			{
		...
		const adRows = await fetchAdSpendRows(
			connection,
			accessToken,
			job.sync_date,
			{
...
async function loadActiveGoogleAdsConnections(): Promise<GoogleAdsMetadataConnection[]> {
	const result = await query<GoogleAdsMetadataConnection>(
		`
      SELECT
        id,
        customer_id,
...
async function fetchGoogleAccessToken(connection: GoogleAdsMetadataConnection): Promise<string> {
	return exchangeRefreshToken(connection as GoogleAdsConnectionSecretRow);
}

async function fetchGoogleMetadataRows<T>(
	connection: GoogleAdsMetadataConnection,
	accessToken: string,
	gaql: string,
): Promise<T[]> {
	return googleAdsSearch<T>({
		connection: connection as GoogleAdsConnectionSecretRow,
		accessToken,
		gaql,
	});
}
...
export async function processGoogleAdsSyncQueue(
	...
	const workerId =
		options.workerId ?? `google-ads-sync-${randomBytes(6).toString("hex")}`;
...
export function createGoogleAdsPublicRouter(): Router {
	const router = Router();
	...
	router.get("/oauth/callback", async (req, res, next) => {
		try {
			...
			if (!payload.code || !payload.state) {
				throw new GoogleAdsHttpError(
					400,
					"google_ads_oauth_invalid_callback",
			...
			const customer = await fetchCustomerMetadata(
			...
			if (customer.customerId !== oauthState.customer_id) {
				throw new GoogleAdsHttpError(
					...
					`Provided customerId ${oauthState.customer_id} does not match resolved customer ${customer.customerId}`,
				);
			}
...
export function createGoogleAdsAdminRouter(): Router {
	...
	router.get("/oauth/start", async (req, res, next) => {
		try {
			...
			const normalizedCustomerId = normalizeGoogleAdsCustomerId(
				payload.customerId,
			);
