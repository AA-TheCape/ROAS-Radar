...
function parseCampaignParameters(
	pageUrl: string,
): Pick<
...
function buildCaptureFromTrackingEvent(
...
function sanitizeTrackingInput(
	input: TrackingRequestBody,
): SanitizedTrackingEventInput {
...
function normalizeAttributionCaptureRequest(
	body: z.infer<typeof attributionCaptureRequestSchema>,
...
async function findExistingTrackingEventByClientEventId(
...
async function findExistingTrackingEventByFingerprint(
...
async function ingestIdentityEdgesBestEffort(
	client: Parameters<typeof withTransaction>[0] extends (
		client: infer T,
	) => Promise<unknown>
		? T
		: never,
...
async function insertTrackingEventForCapture(
	...
	try {
		const insertResult = await client.query<
			{ id: string } & RawPayloadIntegrityRow
		>(
			`
      ...
      `,
		...
		if (
			...
			const existing = await findExistingTrackingEventByFingerprint(
				input.ingestionFingerprint,
			);
	...
	try {
      ...
      `,
		...
		if (
			...
			const existing = await findExistingAttributionTouchEventByFingerprint(
				input.ingestionFingerprint,
			);
...
async function ingestAttributionCapture(
	...
	if (options.precheckDuplicates) {
		const existing =
			await findExistingAttributionTouchEventByFingerprint(
				ingestionFingerprint,
			);
...
async function ingestTrackingEvent(
	...
	const eventId = await withTransaction(async (client) => {
		...
		const insertedEventId = await insertTrackingBrowserEvent(
			client,
			input,
			rawPayload,
			ingestionFingerprint,
...
function cloneTrackingRawPayload(
...
function enforceRateLimit(req: Request): void {
	...
	const body =
		typeof req.body === "object" && req.body !== null
			...
			: {};
	const sessionHint =
		typeof body.sessionId === "string" ? body.sessionId : "anonymous";
...
function parseAttributionCaptureRequest(
	body: unknown,
): AttributionCaptureRequest {
	try {
		return normalizeAttributionCaptureRequest(
			attributionCaptureRequestSchema.parse(body),
		);
	} catch (error) {
		if (error instanceof z.ZodError) {
			throw new TrackingHttpError(
				400,
				"invalid_request",
				"Invalid attribution capture payload",
				error.flatten(),
			);
		}
...
export function createTrackingRouter() {
	...
	router.get("/session", async (req, res, next) => {
		try {
			...
			const headerReferrerUrl = normalizeTrackingUrl(
				req.header("referer") ?? null,
			);
			const normalizedReferrerUrl =
				normalizeTrackingUrl(parsedQuery.referrerUrl) ?? headerReferrerUrl;

			if (!normalizedPageUrl) {
				throw new TrackingHttpError(
					400,
					"invalid_request",
					"pageUrl must be a valid http(s) URL",
				);
			}
			...
			if (error instanceof TrackingHttpError) {
				...
				next(
					new TrackingHttpError(
						400,
						"invalid_request",
						"Invalid session bootstrap request",
						error.flatten(),
					),
				);
				return;
	...
	router.post("/attribution", async (req, res, next) => {
		try {
			...
			const rawPayload = cloneTrackingRawPayload(
				parseTrackingRequestBody(req.body),
			);
	...
	router.post("/", async (req, res, next) => {
		try {
			...
			const { sanitized: sanitizedInput } =
				parseTrackingEventRequest(rawPayload);
			const requestIp = resolveRequestIp(req);
			const browserResult = await ingestTrackingEvent(
				sanitizedInput,
				rawPayload,
				requestIp,
			);
			const serverAttributionResult =
				await emitDerivedAttributionFromBrowserEvent(
