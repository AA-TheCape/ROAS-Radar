...
type MetadataEntityType = 'campaign' | 'adset' | 'ad';

type HistoricalMetadataRow = {
...
export type CampaignMetadataBackfillInput = {
  requestedBy: string;
  workerId: string;
  startDate: string;
  endDate: string;
...
export type CampaignMetadataBackfillReport = {
  ...
  campaignCoverageBefore: CoverageSummary;
...
function endOfReportDate(reportDate: string): Date {
  return new Date(`${reportDate}T23:59:59.000Z`);
}

async function loadHistoricalMetadataRows(startDate: string, endDate: string): Promise<HistoricalMetadataRow[]> {
  const result = await query<HistoricalMetadataRow>(
    `
...
function collapseHistoricalRows(rows: HistoricalMetadataRow[]): CanonicalMetadataRecord[] {
  const records = new Map<string, CanonicalMetadataRecord>();
...
async function loadExistingMetadata(records: CanonicalMetadataRecord[]): Promise<Map<string, PersistedMetadataRow>> {
  if (records.length === 0) {
    return new Map();
  }

  const result = await query<PersistedMetadataRow>(
    `
  ...
  const map = new Map<string, PersistedMetadataRow>();
...
async function upsertMetadata(records: CanonicalMetadataRecord[]): Promise<void> {
...
async function loadRequestedCampaigns(startDate: string, endDate: string): Promise<string[]> {
  const result = await query<{ campaign: string }>(
    `
...
async function emitFreshnessSnapshots(
  ...
  for (const [key, bucket] of buckets) {
    const [platform, entityType] = key.split('\u0000') as [MetadataPlatform, MetadataEntityType];
...
export async function backfillCampaignMetadataHistory(
  input: CampaignMetadataBackfillInput
  ...
  try {
    ...
    for (const record of records) {
      ...
      if (
        collapseWhitespace(existing.latest_name) !== record.latestName ||
    ...
    const unresolvedSamples = unresolvedRecords.slice(0, unresolvedSampleLimit).map<UnresolvedSample>((record) => ({
    ...
    const report: CampaignMetadataBackfillReport = {
      runId,
      status: 'completed',
      startedAt: startedAt.toISOString(),
    ...
    emitCampaignMetadataSyncJobLifecycleLog({
      ...
      plannedInserts,
      plannedUpdates,
      campaignResolvedRate:
        report.campaignCoverageAfter.totalEntities > 0
          ? report.campaignCoverageAfter.resolvedEntities / report.campaignCoverageAfter.totalEntities
          : 0,
      overallUnresolvedRate:
        report.unresolvedRate.totalEntities > 0
