...
function normalizeReportingTimezone(input: string): string {
	...
	try {
		...
		throw new SettingsHttpError(
			400,
			"invalid_timezone",
			"Timezone must be a valid IANA timezone or supported alias",
		);
	}
}
...
export async function getReportingTimezone(
	...
	return normalizeReportingTimezone(
		row.reporting_timezone || DEFAULT_REPORTING_TIMEZONE,
	);
}
...
function serializeSettings(row: AppSettingsRow) {
	return {
		reportingTimezone: normalizeReportingTimezone(
			row.reporting_timezone || DEFAULT_REPORTING_TIMEZONE,
		),
