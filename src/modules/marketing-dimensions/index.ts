...
const CLICK_ID_TYPES = [
	...
	"ttclid",
	"msclkid",
] as const satisfies readonly AttributionClickIdField[];
...
const CLICK_ID_SOURCE_MEDIUM_MAP: Record<
	CanonicalClickIdType,
...
function normalizeFreeformDimension(
	value: string | null | undefined,
): string | null {
...
function normalizeClickIdType(
	value: string | null | undefined,
): CanonicalClickIdType | null {
...
function resolveCanonicalClickId(
	input: CanonicalTouchpointInput | CanonicalSpendInput,
): {
	clickIdType: CanonicalClickIdType | null;
	clickIdValue: string | null;
} {
	...
	const gbraid =
		"gbraid" in input ? normalizeNullableString(input.gbraid) : null;
	if (gbraid) {
		return {
			clickIdType: "gbraid",
			clickIdValue: gbraid,
	...
	const wbraid =
		"wbraid" in input ? normalizeNullableString(input.wbraid) : null;
	if (wbraid) {
		return {
			clickIdType: "wbraid",
			clickIdValue: wbraid,
	...
	const fbclid =
		"fbclid" in input ? normalizeNullableString(input.fbclid) : null;
	if (fbclid) {
		return {
			clickIdType: "fbclid",
			clickIdValue: fbclid,
	...
	const ttclid =
		"ttclid" in input ? normalizeNullableString(input.ttclid) : null;
	if (ttclid) {
		return {
			clickIdType: "ttclid",
			clickIdValue: ttclid,
	...
	const msclkid =
		"msclkid" in input ? normalizeNullableString(input.msclkid) : null;
	if (msclkid) {
		return {
			clickIdType: "msclkid",
...
export function buildCanonicalTouchpointDimensions(
	input: CanonicalTouchpointInput,
): CanonicalTouchpointDimensions {
	const resolvedClickId = resolveCanonicalClickId(input);

	return {
		source: canonicalizeSource(
			normalizeNullableString(input.source),
			resolvedClickId.clickIdType,
		),
		medium: canonicalizeMedium(
			normalizeNullableString(input.medium),
			resolvedClickId.clickIdType,
		),
...
export function buildCanonicalSpendDimensions(
	input: CanonicalSpendInput,
): CanonicalSpendDimensions {
