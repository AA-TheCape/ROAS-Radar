...
type Env = {
  ...
  MMM_READINESS_CAMPAIGN_METADATA_MAX_LAG_HOURS: number;
  MMM_READINESS_CAPTURE_COMPLETENESS_MIN_RATE: number;
  MMM_READINESS_DATA_QUALITY_BLOCKER_LOOKBACK_DAYS: number;
  MMM_READINESS_DUAL_WRITE_MISMATCH_MAX_RATE: number;
  MMM_READINESS_GA4_FALLBACK_MAX_LAG_HOURS: number;
  MMM_READINESS_GA4_FALLBACK_REQUIRED: boolean;
  MMM_READINESS_MISSING_SESSION_ID_MAX_RATE: number;
  MMM_READINESS_OWNER_APPROVERS: string;
  MMM_READINESS_REPORTING_AGGREGATE_MAX_LAG_HOURS: number;
  MMM_READINESS_RESOLVER_UNATTRIBUTED_MAX_RATE: number;
  MMM_READINESS_SPEND_FRESHNESS_MAX_LAG_HOURS: number;
  MMM_READINESS_WINDOW_DAYS: number;
...
const parsers: { [TKey in keyof Env]: EnvParser<Env[TKey]> } = {
  ...
  MMM_READINESS_CAMPAIGN_METADATA_MAX_LAG_HOURS: (name) => parseInteger(name, 48),
  MMM_READINESS_CAPTURE_COMPLETENESS_MIN_RATE: (name) => parseNumber(name, 0.98),
  MMM_READINESS_DATA_QUALITY_BLOCKER_LOOKBACK_DAYS: (name) => parseInteger(name, 7),
  MMM_READINESS_DUAL_WRITE_MISMATCH_MAX_RATE: (name) => parseNumber(name, 0.001),
  MMM_READINESS_GA4_FALLBACK_MAX_LAG_HOURS: (name) => parseInteger(name, 3),
  MMM_READINESS_GA4_FALLBACK_REQUIRED: (name) => parseBoolean(name, false),
  MMM_READINESS_MISSING_SESSION_ID_MAX_RATE: (name) => parseNumber(name, 0.02),
  MMM_READINESS_OWNER_APPROVERS: (name) => parseString(name, ''),
  MMM_READINESS_REPORTING_AGGREGATE_MAX_LAG_HOURS: (name) => parseInteger(name, 24),
  MMM_READINESS_RESOLVER_UNATTRIBUTED_MAX_RATE: (name) => parseNumber(name, 0.1),
  MMM_READINESS_SPEND_FRESHNESS_MAX_LAG_HOURS: (name) => parseInteger(name, 30),
  MMM_READINESS_WINDOW_DAYS: (name) => parseInteger(name, 7),
