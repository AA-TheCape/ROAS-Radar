...
if (!VALID_MODES.has(mode)) {
	console.error(
		`Unsupported attribution test mode "${mode}". Use one of: ${[...VALID_MODES].join(", ")}`,
	);
	process.exit(1);
}
...
const COVERAGE_TESTS = CRITICAL_TESTS.filter(
	(testFile) => testFile !== "test/attribution-e2e.integration.test.ts",
);
...
function runMigrations() {
	const migrationResult = run("npm", ["run", "db:migrate"], {
		stdio: "inherit",
	});
...
function enforceCoverageThresholds(output) {
	...
	for (const file of COVERAGE_TARGETS) {
		...
		if (metrics.line < COVERAGE_THRESHOLDS.line) {
			failures.push(
				`${file}: line ${metrics.line}% < ${COVERAGE_THRESHOLDS.line}%`,
			);
		}

		if (metrics.branch < COVERAGE_THRESHOLDS.branch) {
			failures.push(
				`${file}: branch ${metrics.branch}% < ${COVERAGE_THRESHOLDS.branch}%`,
			);
		}

		if (metrics.funcs < COVERAGE_THRESHOLDS.funcs) {
			failures.push(
				`${file}: funcs ${metrics.funcs}% < ${COVERAGE_THRESHOLDS.funcs}%`,
			);
		}
	}
