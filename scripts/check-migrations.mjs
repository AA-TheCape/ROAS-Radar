...
async function verifyMigrations() {
  ...
  const expectedFiles = (await readdir(migrationsDir)).filter((file) => file.endsWith('.sql')).sort();
  ...
  try {
    ...
    if (appliedCount !== expectedFiles.length) {
      throw new Error(`Expected ${expectedFiles.length} applied migrations, found ${appliedCount}`);
    }
  } finally {
    await client.end();
  }
