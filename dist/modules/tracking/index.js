...
const sanitizedString = (maxLength) => z
    .string()
    .trim()
    .max(maxLength)
    .transform((value) => value);
const nullableSanitizedString = (maxLength) => z
...
async function upsertTrackingSession(input, occurredAt, userAgent, ipHash, client) {
...
async function insertTrackingEvent(client, input, ingestionFingerprint) {
    const params = parseCampaignParameters(input.pageUrl);
    const canonicalDimensions = buildCanonicalTouchpointDimensions({
        source: params.utm_source,
        medium: params.utm_medium,
