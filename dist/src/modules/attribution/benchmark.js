...
import { NORTHBEAM_BENCHMARK_FIXTURES, NORTHBEAM_BENCHMARK_FIXTURE_SET_VERSION } from './benchmark-fixtures.js';
export const NORTHBEAM_BENCHMARK_THRESHOLDS = {
...
function formatChannelKey(source, medium, isDirect) {
    if (isDirect || (!source && !medium)) {
        return 'direct / none';
    }
    return `${source ?? '(unknown source)'} / ${medium ?? '(unknown medium)'}`;
}
function referenceCreditToComparable(fixture, credit) {
...
function buildReferenceRun(fixture, reference) {
    return {
        summary: {
...
function buildActualRun(fixture, modelKey) {
...
function buildComparisons(fixtures) {
    return fixtures.flatMap((fixture) => ATTRIBUTION_MODELS.map((modelKey) => ({
...
function buildModelSlices(comparisons, orderCount) {
    return ATTRIBUTION_MODELS.map((modelKey) => {
        const scoped = comparisons.filter((comparison) => comparison.modelKey === modelKey);
        const actualAttributedRevenueCents = scoped.reduce((sum, comparison) => sum + revenueToCents(comparison.actual.summary.totalRevenueCredited), 0);
        const referenceAttributedRevenueCents = scoped.reduce((sum, comparison) => sum + revenueToCents(comparison.reference.summary.totalRevenueCredited), 0);
        const actualUnattributedOrders = scoped.filter((comparison) => comparison.actual.summary.allocationStatus !== 'attributed').length;
        const referenceUnattributedOrders = scoped.filter((comparison) => comparison.reference.summary.allocationStatus !== 'attributed').length;
        const winnerMismatchRate = percentage(scoped.filter((comparison) => comparison.actual.summary.winnerTouchpointId !== comparison.reference.summary.winnerTouchpointId).length, orderCount);
        const touchpointCountMismatchRate = percentage(scoped.filter((comparison) => comparison.actual.summary.touchpointCountConsidered !== comparison.reference.summary.touchpointCountConsidered).length, orderCount);
        const attributedRevenueDeltaPercent = deltaPercent(actualAttributedRevenueCents, referenceAttributedRevenueCents);
        const unattributedRateDeltaPercentagePoints = percentage(Math.abs(actualUnattributedOrders - referenceUnattributedOrders), orderCount);
        let severity = resolveSeverity(attributedRevenueDeltaPercent, NORTHBEAM_BENCHMARK_THRESHOLDS.modelRevenueDeltaPercent);
        severity = maxSeverity(severity, resolveSeverity(winnerMismatchRate, NORTHBEAM_BENCHMARK_THRESHOLDS.winnerMismatchRate));
        severity = maxSeverity(severity, resolveSeverity(unattributedRateDeltaPercentagePoints, NORTHBEAM_BENCHMARK_THRESHOLDS.unattributedRateDeltaPercentagePoints));
        return {
            modelKey,
...
function buildChannelSlices(comparisons) {
    ...
    for (const comparison of comparisons) {
        for (const credit of comparison.actual.credits) {
            ...
            const entry = channelMap.get(key) ?? {
                ...
                actualOrders: new Set(),
                referenceOrders: new Set()
            };
...
function buildCohortSlices(comparisons) {
    ...
    return cohorts.flatMap((cohort) => ATTRIBUTION_MODELS.map((modelKey) => {
        ...
        const primaryWinnerMismatchRate = percentage(scoped.filter((comparison) => comparison.actual.summary.winnerTouchpointId !== comparison.reference.summary.winnerTouchpointId).length, orderCount);
        const directClassificationMismatchRate = percentage(scoped.filter((comparison) => winnerIsDirect(comparison.actual) !== winnerIsDirect(comparison.reference)).length, orderCount);
        const lookbackEligibilityMismatchRate = percentage(scoped.filter((comparison) => comparison.actual.summary.touchpointCountConsidered !== comparison.reference.summary.touchpointCountConsidered ||
            comparison.actual.summary.lookbackRuleApplied !== comparison.reference.summary.lookbackRuleApplied).length, orderCount);
        const syntheticFallbackUsageMismatchRate = percentage(scoped.filter((comparison) => usesSyntheticFallback(comparison.actual) !== usesSyntheticFallback(comparison.reference))
            .length, orderCount);
        let severity = resolveSeverity(primaryWinnerMismatchRate, NORTHBEAM_BENCHMARK_THRESHOLDS.winnerMismatchRate);
        severity = maxSeverity(severity, resolveSeverity(directClassificationMismatchRate, NORTHBEAM_BENCHMARK_THRESHOLDS.winnerMismatchRate));
        severity = maxSeverity(severity, resolveSeverity(lookbackEligibilityMismatchRate, NORTHBEAM_BENCHMARK_THRESHOLDS.winnerMismatchRate));
        severity = maxSeverity(severity, resolveSeverity(syntheticFallbackUsageMismatchRate, NORTHBEAM_BENCHMARK_THRESHOLDS.winnerMismatchRate));
        return {
            cohort,
            modelKey,
            orderCount,
...
function buildOrderLevelMismatches(comparisons) {
    const mismatches = [];
    for (const comparison of comparisons) {
...
export function buildNorthbeamBenchmarkReport(fixtures = NORTHBEAM_BENCHMARK_FIXTURES) {
...
export function evaluateNorthbeamBenchmarkThresholds(report) {
    const violations = [];
    for (const slice of report.modelSlices) {
        if (slice.severity === 'red') {
            ...
            if (slice.unattributedRateDeltaPercentagePoints >
...
export function formatNorthbeamBenchmarkThresholdFailures(result) {
    if (result.ok) {
    ...
    for (const violation of result.violations) {
        ...
        if (violation.slice === 'channel') {
            lines.push(`- channel ${violation.modelKey} ${violation.channelKey} ${violation.metric}: ${violation.details}`);
            continue;
        }
        lines.push(`- cohort ${violation.cohort} ${violation.modelKey} ${violation.metric}: ${violation.details}`);
    }
    return lines.join('\n');
}
export function assertNorthbeamBenchmarkThresholds(report) {
    const result = evaluateNorthbeamBenchmarkThresholds(report);
    if (!result.ok) {
        throw new Error(formatNorthbeamBenchmarkThresholdFailures(result));
    }
}
export function renderNorthbeamBenchmarkReportJson(report) {
    return `${JSON.stringify(report, null, 2)}\n`;
}
function formatSeverityLabel(severity) {
    return severity.toUpperCase();
}
export function renderNorthbeamBenchmarkReportMarkdown(report) {
    const lines = [];
    ...
    lines.push('| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |');
    for (const slice of report.channelSlices) {
        lines.push(`| ${slice.modelKey} | ${slice.channelKey} | ${formatSeverityLabel(slice.severity)} | ${slice.actualRevenue} | ${slice.referenceRevenue} | ${slice.absoluteRevenueDelta} | ${slice.revenueDeltaPercent}% | ${slice.orderCountDelta} |`);
    }
    lines.push('');
    ...
    lines.push('| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |');
    for (const slice of report.cohortSlices) {
        lines.push(`| ${slice.cohort} | ${slice.modelKey} | ${formatSeverityLabel(slice.severity)} | ${slice.orderCount} | ${slice.primaryWinnerMismatchRate}% | ${slice.directClassificationMismatchRate}% | ${slice.lookbackEligibilityMismatchRate}% | ${slice.syntheticFallbackUsageMismatchRate}% |`);
    }
    lines.push('');
    ...
    if (report.topVarianceDrivers.length === 0) {
        ...
        for (const driver of report.topVarianceDrivers) {
            lines.push(`- ${driver.severity.toUpperCase()} ${driver.modelKey} / ${driver.cohort} / ${driver.fixtureId}: ${driver.mismatchReasons.join(', ')}. Actual winner \`${driver.actualWinnerTouchpointId ?? 'none'}\`, reference winner \`${driver.referenceWinnerTouchpointId ?? 'none'}\`.`);
        }
    }
    lines.push('');
