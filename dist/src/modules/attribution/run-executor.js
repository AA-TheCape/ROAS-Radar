...
function buildEngineTouchpoint(datasetTouchpoint) {
    return {
...
function buildConfidenceLabel(touchpoint) {
...
async function insertExplainRecord(client, input) {
    await client.query(`
...
async function persistBatch(client, run, orderIds) {
    ...
    for (const orderId of orderIds) {
      ...
      `, [
        ...
        const execution = executeAttributionModels(orderTouchpoints.map((touchpoint) => buildEngineTouchpoint(touchpoint)), {
        ...
        `, [
          ...
          `, [
                    ...
                    credit.isPrimary,
                    credit.evidenceSource,
                    buildConfidenceLabel(orderTouchpoints.find((touchpoint) => touchpoint.touchpoint_id === credit.touchpointId) ?? orderTouchpoints[0])
                ]);
                await insertExplainRecord(client, {
                    runId: run.id,
                    orderId,
            ...
            for (const touchpoint of orderTouchpoints) {
                ...
                await insertExplainRecord(client, {
                    ...
                    stage: model === 'hinted_fallback_only' ? 'fallback' : 'model_scoring',
                    decision: 'no_credit',
                    reason: summary.allocationStatus === 'blocked_by_deterministic'
...
export async function executeAttributionRun(options) {
