...
function parseCheckpoints(value) {
    ...
    for (const source of IDENTITY_BACKFILL_SOURCES) {
        ...
        defaults[source] = {
            lastTimestamp: typeof sourceValue.lastTimestamp === "string"
                ? sourceValue.lastTimestamp
                : null,
            lastCursor: typeof sourceValue.lastCursor === "string"
...
export async function backfillHistoricalIdentityGraph(options) {
    ...
    const runId = normalizeNullableString(options.runId) ??
        (await createIdentityGraphBackfillRun({
            requestedBy,
            workerId,
            scope,
        }));
    const maxBatches = options.maxBatches == null
        ? null
