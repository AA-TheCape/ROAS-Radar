...
function writeRuntimeConfig(response) {
	const payload = {
		apiBaseUrl: "",
		reportingToken:
			process.env.DASHBOARD_REPORTING_API_TOKEN ??
			process.env.VITE_REPORTING_API_TOKEN ??
			"",
		reportingTenantId:
			process.env.DASHBOARD_REPORTING_TENANT_ID ??
			process.env.VITE_REPORTING_TENANT_ID ??
			"roas-radar",
	};
	...
	if (shouldProxyToApi(url.pathname)) {
		proxyApiRequest(request, response, url).catch((error) => {
			writeJson(response, 502, {
				error:
					error instanceof Error ? error.message : "unexpected_proxy_error",
			});
		});
		return;
	}
