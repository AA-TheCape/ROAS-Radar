...
function readTrackerConfig(): Required<TrackerConfig> {
	...
	return {
		retryBaseDelayMs:
			windowConfig.retryBaseDelayMs ?? DEFAULT_RETRY_BASE_DELAY_MS,
...
function resolveLandingUrl(pageUrl: string): {
	landingUrl: string;
	isFirstEntry: boolean;
} {
	const existingLandingUrl = normalizeUrl(
		safeGetStorage(LANDING_URL_STORAGE_KEY),
	);
...
function enqueueRetry(payload: TrackingEventPayload, attempt: number): void {
	const queue = getQueue().filter(
		(entry) => entry.payload.clientEventId !== payload.clientEventId,
	);
...
async function requestSession(
	...
	const response = await fetch(
		`${buildApiUrl("/track/session")}?${params.toString()}`,
		{
			method: "GET",
			credentials: "include",
			headers: {
...
function ensureSession(
	...
	const pendingPromise = requestSession(pageUrl, landingUrl, referrerUrl).catch(
		() => fallbackSession(),
	);
