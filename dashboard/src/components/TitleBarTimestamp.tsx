...
export default function TitleBarTimestamp({
  ...
  }, [clearScheduledInterval, clearScheduledTimeout, getNow, scheduleInterval, scheduleTimeout]);

  const labels = useMemo(
    () => ({
      local: formatCurrentTimestamp(currentTime, { includeTimeZoneName: true }),
      utc: formatCurrentTimestamp(currentTime, { timeZone: 'UTC', includeTimeZoneName: true }).replace(/ UTC$/, '')
    }),
    [currentTime]
  );

  return (
    <div aria-label="Current timestamp" className="space-y-1 border-t border-line/60 pt-3 text-caption text-ink-muted">
      <p className="font-semibold uppercase tracking-[0.14em] text-teal">Current time</p>
      <p>{labels.local}</p>
      <p>UTC {labels.utc}</p>
    </div>
  );
