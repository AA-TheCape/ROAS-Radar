...
export function formatPieMetric(
	value: number,
...
const ChartTooltip = memo(function ChartTooltip({
	title,
	rows,
}: { title: string; rows: ChartTooltipRow[] }) {
	return (
			...
			<div className="mt-3 grid gap-2">
				{rows.map((row) => (
					<div
						...
						className="flex items-center justify-between gap-4 text-body text-ink-soft"
					>
...
const ChartFrame = memo(function ChartFrame({
	...
	const titleId = useId();
	const descriptionId =
		description || summary ? `${titleId}-description` : undefined;
			...
			<figcaption className="sr-only">
				...
				{description || summary ? (
					<span id={descriptionId}>
						{[description, typeof summary === "string" ? summary : null]
							.filter(Boolean)
							.join(" ")}
					</span>
				) : null}
			...
			<ResponsiveLine
				...
				axisLeft={{
					...
					format: (value: number | string) =>
						formatMetric(Number(value), yFormat),
				}}
...
export const NivoAreaChart = memo(function NivoAreaChart({
	...
	const { isCompact, isTablet } = useChartViewport();
	const effectiveLegends = isCompact ? [] : legends;
	const effectiveHeight = isCompact
		? Math.max(260, (props.height ?? 320) - 36)
		: props.height;
	const effectiveMargin = useMemo(
		() => ({
			...baseLineMargin,
	...
	return (
		<ChartFrame
			...
			empty={
				props.empty ??
				(props.data.length === 0 ||
					props.data.every((series) => series.data.length === 0))
			}
		...
		>
			<ResponsiveLine
				...
				axisLeft={{
					...
					legendPosition: "middle",
					format: (value: number | string) =>
						formatMetric(Number(value), props.yFormat),
				}}
				defs={[
					{
						id: "areaGradient",
	...
	return (
		...
		>
			<ResponsiveBar
				...
				valueFormat={(value: string | number) =>
					formatMetric(Number(value), valueFormat)
				}
				legends={effectiveLegends}
				tooltip={({
					id,
...
export const NivoPieChart = memo(function NivoPieChart({
	...
	const pieValueFormat = useMemo(
		() => (value: number | string) =>
			formatPieMetric(Number(value), valueFormat),
		[valueFormat],
	);
	const effectiveMargin = useMemo(
		() => ({
