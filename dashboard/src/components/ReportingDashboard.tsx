...
const chartSuspenseFallback = (
	<div
		className="min-h-[280px] animate-pulse rounded-card border border-line/60 bg-surface-alt/70"
		aria-hidden="true"
	/>
);
...
export function applyDateRangeChange(
	...
	if (
		...
		return {
			nextFilters,
			feedback:
				"End date was adjusted to match the new start date so the range stays valid.",
		};
	}
	...
	if (
		...
		return {
			nextFilters,
			feedback:
				"End date cannot be earlier than the start date. It was moved forward to keep the range valid.",
		};
	}
...
const SummaryCard = memo(function SummaryCard({
	...
	return (
		...
		>
			<div
				className={`absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r ${decor.accent}`}
			/>
...
function buildTrendSummary(
	...
	const strongestPoint = points.reduce<TimeseriesPoint>(
		(current, point) => (point.revenue > current.revenue ? point : current),
		points[0],
	);
...
function buildCampaignMixSummary(
	data: Array<{ campaign: string; revenueShare: number; revenue: number }>,
) {
	if (data.length === 0) {
	...
	return data
		...
		.map(
			(row) =>
				`${row.campaign}: ${row.revenueShare.toFixed(1)} percent share from ${formatCurrency(row.revenue)}.`,
		)
		.join(" ");
}
...
function buildSourceMixSummary(
	...
	return data
		.slice(0, 5)
		.map(
			(row) => `${row.id}: ${row.revenueLabel ?? formatCurrency(row.value)}.`,
		)
		.join(" ");
}
...
const DashboardOverview = memo(function DashboardOverview({
	...
	const peakPoint = useMemo(
		() =>
			points.reduce<TimeseriesPoint | null>(
				(current, point) =>
					!current || point.revenue > current.revenue ? point : current,
				null,
			),
		[points],
	);
	...
	const leadingCampaign = useMemo(
		() =>
			campaigns.reduce<CampaignRow | null>(
				(current, row) =>
					!current || row.revenue > current.revenue ? row : current,
				null,
			),
		[campaigns],
	);
	...
	return (
		...
		>
			<div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.85fr)]">
				...
				>
					...
					<div className="relative grid gap-6">
						<div className="flex flex-wrap items-center justify-between gap-3">
							<div>
								...
								<CardDescription className="mt-4 max-w-2xl text-lead">
									...
									, aligned to {reportingTimezone}, and tied to the existing
									summary, campaign, timeseries, and order feeds.
								</CardDescription>
							</div>
						...
						<div className="grid gap-4 md:grid-cols-3">
							<Card
								padding="compact"
								className="border-white/70 bg-white/80 shadow-inset-soft"
							>
								...
								<p className="mt-2 text-body text-ink-soft">
									{peakPoint
										? formatCurrency(peakPoint.revenue)
										: "No revenue peak available yet"}
								</p>
							</Card>
							...
							>
								...
								<p className="mt-2 text-body text-ink-soft">
									{points.length > 0
										? `${formatNumber(points.length)} reporting buckets in view`
				...
				<Card
					padding="card"
					className="bg-[linear-gradient(180deg,rgba(23,33,43,0.98),rgba(49,64,81,0.96))] text-white"
				>
					...
					<div className="mt-5 grid gap-4">
						<div className="rounded-card border border-white/10 bg-white/5 px-4 py-4">
							<p className="text-caption uppercase tracking-[0.14em] text-white/80">
								Revenue captured
							</p>
							<p className="mt-3 font-display text-display text-white">
								{formatCompactCurrency(totalRevenue)}
							</p>
							<p className="mt-2 text-body text-white/88">
								{summary?.roas == null
									? "ROAS pending spend data"
									: `${formatNumber(summary.roas)} ROAS on attributed revenue`}
							</p>
						</div>
						<div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
							<div className="rounded-card border border-white/10 bg-white/5 px-4 py-4">
								...
								<p className="mt-2 font-display text-title text-white">
...
const DashboardControlPanel = memo(function DashboardControlPanel({
	...
	function handleDateChange(field: DateField, value: string) {
		const { nextFilters, feedback } = applyDateRangeChange(
			filters,
			field,
			value,
		);
				...
				<div className="flex flex-wrap items-start justify-between gap-3">
					<div className="min-w-0">
						...
						<div className="mt-2 flex flex-wrap items-center gap-2">
							...
							<Badge
								tone="teal"
								className="min-h-[26px] px-2.5 py-0.5 text-[0.68rem]"
							>
								{reportingTimezone}
							</Badge>
						</div>
						<p className="mt-2 max-w-3xl text-[0.95rem] leading-6 text-ink-muted">
							Tune dates, source filters, and chart grouping without giving the
							dashboard a full-height control section.
						</p>
					</div>
					...
					<div className="flex flex-wrap items-center gap-2">
						<Badge
							...
							className="min-h-[26px] px-2.5 py-0.5 text-[0.68rem]"
						>
							{GROUP_BY_OPTIONS.find((option) => option.value === groupBy)
								?.label ?? groupBy}
						</Badge>
						<Button
							type="button"
					...
					<div className="grid gap-3 rounded-card border border-line/60 bg-white/75 px-3 py-3 shadow-inset-soft">
						<div className="flex flex-wrap items-center gap-2">
							{quickRanges.map((preset) => (
								<Button
									...
									onClick={() =>
										onApplyQuickRange(preset.value(reportingTimezone))
									}
								>
									{preset.label}
								</Button>
						...
						<div className="flex flex-wrap items-center justify-between gap-2">
							<div className="min-w-0">
								...
								<p className="mt-1 truncate text-[0.95rem] text-ink-soft xl:max-w-[22rem]">
									{scopeLabel}
								</p>
						...
						{dateFeedback ? (
							<output
								className="text-[0.85rem] font-medium text-danger"
								aria-live="polite"
							>
								{dateFeedback}
							</output>
...
const TimeseriesTrendPanel = memo(function TimeseriesTrendPanel({
	...
	const rankedPoints = useMemo(
		() => [...points].sort((left, right) => left.revenue - right.revenue),
		[points],
	);
	const topBuckets = useMemo(
		() => rankedPoints.slice(-3).reverse(),
		[rankedPoints],
	);
	...
	const averageBucketRevenue = useMemo(
		() =>
			points.length > 0
				? points.reduce((sum, point) => sum + point.revenue, 0) / points.length
				: 0,
		[points],
	);
					...
					<CardHeader className="items-center">
						<div>
							...
							<CardDescription className="mt-2">
								Revenue remains isolated from tables so grouping changes do not
								disturb campaign or order widgets.
							</CardDescription>
						</div>
						...
						<div className="mt-4 grid gap-4 sm:grid-cols-3 xl:grid-cols-1">
							<div>
								...
								<p className="mt-2 font-display text-title text-ink">
									{formatCompactCurrency(averageBucketRevenue)}
								</p>
							</div>
							<div>
								<p className="text-label uppercase text-ink-muted">
							...
							<div className="flex items-start justify-between gap-3">
								<div>
									...
									<p className="mt-2 text-body text-ink-muted">
										Highest revenue buckets in this view.
									</p>
								</div>
									...
									topBuckets.map((point) => (
										...
										>
											...
											<p className="mt-1 text-caption text-ink-muted">
												{formatNumber(point.orders)} orders from{" "}
						...
						>
							<div className="flex items-start justify-between gap-3">
								<div>
									...
									<p className="mt-2 text-body text-ink-muted">
										Lowest revenue buckets in this view.
									</p>
								</div>
				...
				revenueShare: Number(
					(
						(totalCampaignRevenue > 0
							? row.revenue / totalCampaignRevenue
							: 0) * 100
					).toFixed(2),
				),
				revenue: row.revenue,
						...
						<>
							...
							<DataTableToolbar
								...
								summary={
									<>
										<TableMeta
											currentCount={filteredCampaigns.length}
								...
								<Table caption="Campaign performance">
									...
									<TableBody>
										...
										{paginatedCampaigns.rows.map((row) => (
											<TableRow
												key={`${row.source}-${row.medium}-${row.campaign}-${row.content ?? "none"}`}
											>
												<TableCell>
													<PrimaryCell>
						...
						<div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
							<div>
								...
								<p className="mt-3 max-w-2xl text-body text-ink-soft">
									Channel groupings follow the same reporting window and
									dashboard filters as the summary totals.
								</p>
							</div>
							<div className="grid gap-2 rounded-card border border-line/60 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(220,239,237,0.52))] px-4 py-4 text-right shadow-inset-soft">
								...
								<p className="font-display text-title text-brand">
									{formatCurrency(totalGroupedSpend)}
								</p>
								<p className="text-body text-ink-muted">
									{formatNumber(spendGroups.length)} channels in the current
									window
								</p>
							</div>
						</div>
								...
								>
									<div className="flex flex-wrap items-start justify-between gap-3">
										<div>
											...
											<p className="mt-3 font-display text-title text-ink">
												{formatChannelLabel(group.source, group.medium)}
											</p>
											<p className="mt-2 text-body text-ink-muted">
												{formatNumber(group.campaigns.length)} campaigns
												contributing spend
											</p>
										</div>
										<Badge tone="teal">
											{formatCurrency(group.subtotal)} subtotal
										</Badge>
									</div>

									<TableWrap className="mt-5">
										<Table
											caption={`Spend detail for ${formatChannelLabel(group.source, group.medium)}`}
										>
											<TableHead>
												<TableRow>
											...
											<TableBody>
												{group.campaigns.map((campaign) => (
													<TableRow
														key={`${group.source}-${group.medium}-${campaign.campaign}`}
													>
														<TableCell>
															<PrimaryCell>
