...
export default function OrderDetailsView({
	...
	const attributedRevenue = attributionCredits.reduce(
		(sum, credit) => sum + credit.revenueCredit,
		0,
	);
	...
	const paginatedLineItems = useMemo(
		() => paginateRows(sortedLineItems, lineItemPage, 6),
		[lineItemPage, sortedLineItems],
	);
	...
	const paginatedCredits = useMemo(
		() => paginateRows(sortedCredits, creditPage, 6),
		[creditPage, sortedCredits],
	);

	function toggleLineItemSort(
		key: "title" | "sku" | "quantity" | "price" | "vendor",
	) {
              ...
              <div>
                ...
                <dd>{formatOptionalValue(order?.fulfillmentStatus)}</dd>
					...
					<DetailCard
						...
						description="Identity stitching fields captured across Shopify and ROAS Radar."
					>
						<DetailList className="xl:grid-cols-2">
							<div>
							...
							<div>
								...
								<dd className="break-all">
									{formatOptionalValue(order?.emailHash)}
								</dd>
							</div>
							<div>
								<dt>Landing session ID</dt>
								<dd className="break-all">
									{formatOptionalValue(order?.landingSessionId)}
								</dd>
							</div>
							<div>
								<dt>Checkout token</dt>
								<dd className="break-all">
									{formatOptionalValue(order?.checkoutToken)}
								</dd>
							</div>
							<div className="md:col-[1/-1]">
								<dt>Cart token</dt>
								<dd className="break-all">
									{formatOptionalValue(order?.cartToken)}
								</dd>
							</div>
							<div>
					...
					<DetailCard
						title="Timestamps"
						description="Operational dates shown in the active reporting timezone."
					>
						<DetailList className="grid-cols-1">
							...
							<div>
								<dt>Ingested</dt>
								<dd>
									{formatOptionalDateTime(order?.ingestedAt, reportingTimezone)}
								</dd>
							</div>
						</DetailList>
					</DetailCard>
				...
				<DetailCard
					title="Line items"
					description="Commercial line-item detail from Shopify, including variant metadata and ingestion flags."
				>
					<DataTableToolbar
						...
						summary={
							<>
								<TableMeta
									currentCount={filteredLineItems.length}
									...
									<TableRow key={item.shopifyLineItemId}>
										...
										<TableCell>
											<div className="grid gap-1 text-body text-ink-muted">
												<span>
													{formatOptionalValue(item.fulfillmentStatus)}
												</span>
												<span>
													Shipping {formatOptionalValue(item.requiresShipping)}{" "}
													· Taxable {formatOptionalValue(item.taxable)}
												</span>
											</div>
										</TableCell>
									</TableRow>
				...
				<DetailCard
					...
					description="Per-touchpoint revenue allocation stored for this order across attribution models."
				>
					<DataTableToolbar
						...
						summary={
							<>
								<TableMeta
									currentCount={filteredCredits.length}
									totalCount={attributionCredits.length}
									label="credits"
								/>
								<TablePagination
						...
						<Table caption="Attribution credits">
							...
							<TableBody>
								...
								{paginatedCredits.rows.map((credit) => (
									<TableRow
										key={`${credit.attributionModel}-${credit.touchpointPosition}-${credit.sessionId ?? "none"}`}
									>
										<TableCell>
											<PrimaryCell>
												...
												<span>
													{credit.isPrimary
														? "Primary touchpoint"
														: "Supporting touchpoint"}
												</span>
											</PrimaryCell>
										</TableCell>
										<TableCell>
											{formatNumber(credit.touchpointPosition)}
										</TableCell>
										<TableCell>
											<PrimaryCell className="gap-0.5">
												<strong>{`${credit.source ?? "Unknown"} / ${credit.medium ?? "Unknown"}`}</strong>
												<span>
													{credit.clickIdType && credit.clickIdValue
														? `${credit.clickIdType}: ${credit.clickIdValue}`
														: "No click ID stored"}
												</span>
											</PrimaryCell>
										...
										<TableCell>
											{formatCurrency(credit.revenueCredit)}
										</TableCell>
										<TableCell>{formatNumber(credit.creditWeight)}</TableCell>
										<TableCell>
											<div className="flex flex-wrap items-center gap-2">
												<Badge tone="teal">
													{formatContractValue(credit.matchSource)}
												</Badge>
												<Badge tone="brand">
													{formatContractValue(credit.confidenceLabel)}
												</Badge>
												<Badge
													tone="brand"
				...
				<div className="grid gap-4 xl:grid-cols-2">
					<DetailCard
						...
						description="Persisted winner and timeline snapshot written onto the Shopify order record."
					>
						<JsonViewer value={order?.attributionSnapshot ?? {}} />
					</DetailCard>

					<DetailCard
						title="Raw order payload"
						description="Stored Shopify payload for the order record."
					>
						<JsonViewer value={order?.rawPayload ?? {}} />
					</DetailCard>

					<DetailCard
						title="Raw line item payloads"
						description="Per-line-item Shopify payloads, grouped in insertion order."
					>
						{lineItems.length === 0 ? (
							<EmptyState
							...
							<div className="grid gap-4">
								{lineItems.map((item, index) => (
									<div
										key={`${item.shopifyLineItemId}-raw`}
										...
										<div className="flex flex-col gap-3 rounded-card border border-line/60 bg-surface-alt/60 px-4 py-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
											<div>
												...
												<p className="text-body text-ink-muted">
													{formatOptionalValue(item.variantTitle)} · Qty{" "}
													{formatNumber(item.quantity)}
												</p>
											</div>
											<Tooltip content="Shopify line item identifier">
												<StatusPill>
													{formatOptionalValue(item.shopifyLineItemId)}
												</StatusPill>
											</Tooltip>
										</div>
