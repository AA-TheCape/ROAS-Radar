...
function MetricCard({
	...
	return (
		<Card
			padding="compact"
			className="min-h-[11.5rem] border-line/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(240,246,242,0.86))]"
		>
...
function DailySeriesSummary({
	...
	return (
		<div className="grid gap-3">
			{points.map((point) => {
				const linkedWidth = Math.max(
					4,
					Math.round((point.linked / maxValue) * 100),
				);
				const conflictWidth = Math.max(
					0,
					Math.round((point.conflicts / maxValue) * 100),
				);

				return (
					<div
						key={point.date}
						className="grid gap-2 rounded-card border border-line/60 bg-surface-alt/55 p-4"
					>
						<div className="flex flex-wrap items-center justify-between gap-3">
							<div>
								...
								<p className="text-body text-ink-muted">
									{formatNumber(point.linked)} linked,{" "}
									{formatNumber(point.conflicts)} conflicts,{" "}
									{formatNumber(point.mergeRuns)} merge runs
								</p>
							</div>
...
function ConflictDrilldown({
	...
	const selectedConflict = useMemo(
		() =>
			conflicts.find((conflict) => conflict.edgeId === selectedEdgeId) ??
			conflicts[0] ??
			null,
					...
					<tbody>
						{conflicts.map((conflict) => {
							...
							return (
								<tr
									key={conflict.edgeId}
									className={selected ? "bg-brand-soft/45" : undefined}
								>
									<td>
										<button
											type="button"
										...
										>
											<strong className="text-ink">
												{titleizeToken(conflict.conflictCode)}
											</strong>
											<span className="text-ink-muted">
												{titleizeToken(conflict.evidenceSource)} from{" "}
												{conflict.sourceTable ?? "unknown source"}
											</span>
										</button>
									</td>
									<td>
										<div className="grid gap-1">
											<span className="font-semibold text-ink">
												{titleizeToken(conflict.nodeType)}
											</span>
									...
									<td>
										<div className="grid gap-2">
											...
											<Badge
												tone={formatConflictBadgeTone(conflict.journeyStatus)}
											>
												{conflict.journeyStatus}
											</Badge>
										</div>
									</td>
									<td>
										{formatDateTimeLabel(conflict.updatedAt, reportingTimezone)}
									</td>
								</tr>
							);
						})}
				...
				<CardHeader>
					<div>
						...
						<CardTitle className="mt-3">
							{titleizeToken(selectedConflict?.conflictCode ?? "Conflict")}
						</CardTitle>
						<CardDescription className="mt-3">
							Inspect the affected node, owning journey, and source record
							before repair or replay.
						</CardDescription>
					</div>
				</CardHeader>
				...
				{selectedConflict ? (
					<dl className="grid gap-4 text-body text-ink-soft [&_dt]:text-caption [&_dt]:font-semibold [&_dt]:uppercase [&_dt]:tracking-[0.14em] [&_dt]:text-ink-muted">
						...
						<div>
							...
							<dd>
								{selectedConflict.authoritativeShopifyCustomerId ??
									"None attached"}
							</dd>
						</div>
						<div>
							<dt>Node</dt>
							<dd>
								{titleizeToken(selectedConflict.nodeType)}:{" "}
								{selectedConflict.nodeKey}
							</dd>
						</div>
						<div>
							<dt>Source record</dt>
							<dd>
								{selectedConflict.sourceTable ?? "unknown"} /{" "}
								{selectedConflict.sourceRecordId ?? "n/a"}
							</dd>
						</div>
						<div>
						...
						<div>
							...
							<dd>
								{formatDateTimeLabel(
									selectedConflict.firstObservedAt,
									reportingTimezone,
								)}
							</dd>
						</div>
						<div>
							...
							<dd>
								{formatDateTimeLabel(
									selectedConflict.lastObservedAt,
									reportingTimezone,
								)}
							</dd>
						</div>
	...
	return (
		<section className="grid gap-section">
					...
					<Card tone="teal" className="overflow-hidden">
						<div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
							<div>
								...
								<CardTitle className="mt-3">
									Identity stitch telemetry
								</CardTitle>
								<CardDescription className="mt-3">
									Filter by UTC date range and evidence source to inspect merge
									volume, conflicts, and session linkage pressure.
								</CardDescription>
							</div>
						...
						<div className="grid gap-6">
							<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
								...
								<MetricCard
									label="Backfill runs"
									value={formatNumber(
										overviewSection.data?.backfill.completedRuns,
									)}
								...
								<Card>
									<CardHeader>
										<div>
											...
											<CardTitle className="mt-3">
												Merge and conflict volume
											</CardTitle>
											<CardDescription className="mt-3">
												Daily linked outcomes, conflict spikes, and quarantine
												volume for the selected filter window.
											</CardDescription>
										</div>
									</CardHeader>
									<DailySeriesSummary
										points={overviewSection.data?.series ?? []}
								...
								<Card tone="accent">
									<CardHeader>
										<div>
											...
											<CardTitle className="mt-3">
												Latest identity graph run
											</CardTitle>
											<CardDescription className="mt-3">
												Use this to confirm whether backfill is still processing
												or whether the latest run needs manual repair.
											</CardDescription>
										</div>
									</CardHeader>
									...
									{latestRun ? (
										<dl className="grid gap-4 text-body text-ink-soft [&_dt]:text-caption [&_dt]:font-semibold [&_dt]:uppercase [&_dt]:tracking-[0.14em] [&_dt]:text-ink-muted">
											...
											<div>
												<dt>Sources</dt>
												<dd>
													{latestRun.sources.length > 0
														? latestRun.sources.join(", ")
			...
			>
				...
				>
					<ConflictDrilldown
						conflicts={conflictsSection.data?.conflicts ?? []}
						reportingTimezone={reportingTimezone}
					/>
				</SectionState>
			</Panel>
