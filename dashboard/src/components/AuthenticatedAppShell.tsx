				...
				return (
					...
					>
						...
						<p
							className={[
								"mt-2 text-body",
								active ? "text-white/78" : "text-ink-muted",
							].join(" ")}
						>
							{item.description}
						</p>
...
export function AppShellBreadcrumbs({ items }: BreadcrumbsProps) {
	return (
		<nav
			...
			className="flex flex-wrap items-center gap-2 text-body text-ink-muted"
		>
			{items.map((item, index) => (
				<div key={`${item.label}-${index}`} className="flex items-center gap-2">
					{item.onClick && !item.current ? (
						...
						<span
							className={item.current ? "font-semibold text-ink" : undefined}
						>
							{item.label}
						</span>
					)}
...
export default function AuthenticatedAppShell({
	...
	return (
		<div className="relative min-h-screen bg-canvas text-ink">
			...
			<header className="sticky top-0 z-40 border-b border-line/70 bg-canvas/90 backdrop-blur">
				<div className="mx-auto flex w-full max-w-[92rem] items-center justify-between gap-4 px-gutter py-4 sm:px-section lg:px-section-lg">
					<div className="flex items-center gap-3">
						...
						<div>
							<p className="text-caption font-semibold uppercase tracking-[0.14em] text-teal">
								Authenticated UI
							</p>
					...
					<div className="ml-auto flex items-center gap-3 sm:gap-4">
						<div className="hidden max-w-[28rem] text-right text-body text-ink-muted sm:block">
							{topbarMeta}
						</div>
						{headerActions ? (
					...
					>
						<div className="mb-6 rounded-shell border border-line/80 bg-surface p-panel shadow-panel">
							...
							<p className="mt-2 text-body text-ink-soft">
								Switch between reporting, settings, and drill-in surfaces
								without relying on the old page wrapper.
							</p>
						</div>
						<ShellNavigation
							navItems={navItems}
			...
			<main
				id="app-shell-main"
				className="mx-auto w-full max-w-[92rem] px-gutter py-gutter sm:px-section sm:py-section lg:px-section-lg lg:py-section-lg"
				tabIndex={-1}
			>
