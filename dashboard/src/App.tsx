...
const ReportingDashboard = lazy(
	() => import("./components/ReportingDashboard"),
);
...
const IdentityGraphHealthView = lazy(
	() => import("./components/IdentityGraphHealthView"),
);
...
const AUTHENTICATED_NAV_ITEMS: AppShellNavItem[] = [
	...
	},
	{
		...
		description:
			"Merge activity, conflict drill-down, unlinked session pressure, and identity graph backfill status.",
	},
	{
		key: "settings",
		label: "Settings",
...
export function readDashboardStateFromSearch(
	...
	return {
		filters: normalizeReportingFilters({
			...
			source: source ?? "",
			campaign: campaign ?? "",
			attributionModel: isAttributionModel(attributionModel)
				? attributionModel
				: undefined,
		}),
...
function App() {
	...
	const [appSettings, setAppSettings] = useState<AsyncSection<AppSettings>>(
		createLoadingSection(),
	);
	const [settingsForm, setSettingsForm] = useState<SettingsForm>({
		reportingTimezone: DEFAULT_REPORTING_TIMEZONE,
	});
	const [usersSection, setUsersSection] = useState<AsyncSection<AuthUser[]>>(
		createLoadingSection(),
	);
	const [orderDetailsSection, setOrderDetailsSection] = useState<
		AsyncSection<OrderDetailsResponse>
	>({
		data: null,
		loading: false,
		error: null,
	...
	const [groupBy, setGroupBy] = useState<TimeseriesGroupBy>(
		initialDashboardState.groupBy,
	);
	const [currentPage, setCurrentPage] = useState<AppPage>("dashboard");
	const [dashboardRefreshKey, setDashboardRefreshKey] = useState(0);
	const [identityHealthFilters, setIdentityHealthFilters] =
		useState<IdentityHealthFilters>({
			...buildRange(30, DEFAULT_REPORTING_TIMEZONE),
			source: "",
		});
	const [identityHealthOverview, setIdentityHealthOverview] = useState<
		AsyncSection<IdentityHealthOverviewResponse>
	>({
		data: null,
		loading: false,
		error: null,
	});
	const [identityHealthConflicts, setIdentityHealthConflicts] = useState<
		AsyncSection<IdentityConflictsResponse>
	>({
		data: null,
		loading: false,
		error: null,
	...
	const [shopifyConnection, setShopifyConnection] = useState<
		AsyncSection<ShopifyConnectionResponse>
	>(createLoadingSection());
	...
	const [orderAttributionBackfillJob, setOrderAttributionBackfillJob] =
		useState<AsyncSection<OrderAttributionBackfillJobResponse>>({
			data: null,
			loading: false,
			error: null,
		});
	const [metaConnection, setMetaConnection] = useState<
		AsyncSection<MetaConnectionState>
	>(createLoadingSection());
	...
	const [googleConnection, setGoogleConnection] = useState<
		AsyncSection<GoogleConnectionState>
	>(createLoadingSection());
	...
	const reportingTimezone =
		...
		settingsForm.reportingTimezone ??
		DEFAULT_REPORTING_TIMEZONE;
		...
		try {
			...
			const message =
				error instanceof Error
					? error.message
					: "Failed to load identity health metrics";
	...
	const loadConnections = useCallback(async () => {
		...
		try {
			...
			setMetaConfigForm((current) => ({
				...
				appScopes: metaStatus.config.appScopes.length
					? metaStatus.config.appScopes.join(", ")
					: current.appScopes,
			...
			setGoogleConfigForm((current) => ({
				...
				appScopes: googleStatus.config.appScopes.length
					? googleStatus.config.appScopes.join(", ")
					: current.appScopes,
			}));
		} catch (error) {
			const message =
				error instanceof Error
					? error.message
					: "Failed to load ad connection state";
	...
	const loadOrderAttributionBackfillJob = useCallback(
		async (jobId: string, options?: { preserveData?: boolean }) => {
			...
			try {
				...
				setShopifyOrderAttributionBackfillOptions(
					toBackfillOptionState(response.options),
				);
				...
				setOrderAttributionBackfillJob((current) => ({
					...
					error:
						error instanceof Error
							? error.message
							: "Failed to load order attribution backfill job",
				}));
			}
		},
		[],
	...
	useEffect(() => {
		...
		loadOrderAttributionBackfillJob,
		orderAttributionBackfillJob.data,
	]);
	...
	useEffect(() => {
		...
		const nextSearch = applyDashboardStateToSearch(
			window.location.search,
			filters,
			groupBy,
		);
	...
	const summaryCards = useMemo(() => {
		...
		return [
			...
			},
			{
				...
				detail:
					totals?.roas == null
						? "ROAS pending spend data"
						: `${formatNumber(totals.roas)} ROAS`,
			},
			{
				label: "Spend",
	...
	async function handleMetaConnect() {
		...
		try {
			if ((metaConnection.data?.config.missingFields.length ?? 0) > 0) {
				throw new Error(
					"Save the Meta Ads configuration first. Some required fields are still missing.",
				);
			}
			...
			setActionFeedback({
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to start Meta Ads OAuth",
				message: null,
			});
		}
	}
		...
		try {
			...
			setActionFeedback({
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to save Meta Ads configuration",
				message: null,
			});
		}
	}
		...
		try {
			...
			setActionFeedback({
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to verify Shopify connection",
				message: null,
			});
		}
	}
		...
		try {
			...
			setActionFeedback({
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to sync Shopify webhooks",
				message: null,
			});
		}
	}
		...
		try {
			...
			setActionFeedback({
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to backfill Shopify orders",
				message: null,
			});
		}
	}
	...
	async function handleShopifyAttributionRecovery() {
		...
		try {
			const response: ShopifyAttributionRecoveryResponse =
				await recoverShopifyAttributionHints(
					shopifyBackfillRange.startDate,
					shopifyBackfillRange.endDate,
				);
			await loadConnections();
			...
			setActionFeedback({
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to recover Shopify attribution hints",
				message: null,
			});
		}
	}
	...
	async function handleShopifyOrderAttributionBackfill() {
		...
		if (!parsedRequest.success) {
			...
			setActionFeedback({
				...
				error:
					firstIssue?.message ??
					"Enter a valid date range and limit before queueing the order attribution backfill.",
				message: null,
			});
			return;
		}
		...
		try {
			const response: OrderAttributionBackfillEnqueueResponse =
				await enqueueOrderAttributionBackfill(parsedRequest.data);
			...
			void loadOrderAttributionBackfillJob(response.jobId, {
				preserveData: true,
			});
		} catch (error) {
			setActionFeedback({
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to queue order attribution backfill",
				message: null,
			});
		}
	}
		...
		try {
			...
			setActionFeedback({
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to save Google Ads config",
				message: null,
			});
		}
	}
	...
	async function handleGoogleConnect(event: FormEvent<HTMLFormElement>) {
		...
		try {
			if ((googleConnection.data?.config.missingFields.length ?? 0) > 0) {
				throw new Error(
					"Save the Google Ads configuration first. Some required fields are still missing.",
				);
			}
			...
			setActionFeedback({
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to start Google Ads OAuth",
				message: null,
			});
		}
	}
		...
		try {
			...
			setActionFeedback({
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to queue Google Ads sync",
				message: null,
			});
		}
	}
		...
		try {
			...
			setActionFeedback({
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to reconcile Google Ads",
				message: null,
			});
		}
	}
		...
		currentPage === "order-details"
			? [
					...(isAdmin
						...
						: AUTHENTICATED_NAV_ITEMS.filter(
								(item) => item.key !== "identity-health",
							)),
					{
						key: "order-details",
						label: "Order details",
						shortLabel: "Order",
						description:
							"Contextual drill-in for a selected attributed Shopify order.",
					},
				]
			: isAdmin
				? AUTHENTICATED_NAV_ITEMS
	...
	const breadcrumbs: AppShellBreadcrumb[] =
		currentPage === "dashboard"
			? [{ label: "Authenticated app" }, { label: "Dashboard", current: true }]
	...
	const handleDashboardGroupByChange = useCallback(
		(value: TimeseriesGroupBy) => {
			setGroupBy(value);
		},
		[],
	);
	const handleApplyQuickRange = useCallback(
		(range: Pick<ReportingFilters, "startDate" | "endDate">) => {
			startTransition(() => {
				setFilters((current) => ({
		...
		return (
			...
			>
				<Form onSubmit={(event) => void handleLogin(event)}>
					...
					{authState.error ? (
						<Banner tone="error">{authState.error}</Banner>
					) : null}
					<ButtonRow>
			...
			{currentPage === "dashboard" ? (
				...
				>
					<ReportingDashboard
						...
						onOpenOrderDetails={(shopifyOrderId) =>
							void openOrderDetails(shopifyOrderId)
						}
					/>
				</Suspense>
			) : null}
			...
			{currentPage === "settings" ? (
				...
				>
					<SettingsAdminView
						...
						setNewUserForm={(updater) =>
							setNewUserForm((current) => updater(current))
						}
						shopifyConnection={shopifyConnection}
						shopifyBackfillRange={shopifyBackfillRange}
						setShopifyBackfillRange={(updater) =>
							setShopifyBackfillRange((current) => updater(current))
						}
						shopifyOrderAttributionBackfillOptions={
							shopifyOrderAttributionBackfillOptions
						}
						...
						setMetaConfigForm={(updater) =>
							setMetaConfigForm((current) => updater(current))
						}
						googleConnection={googleConnection}
						googleConfigForm={googleConfigForm}
						setGoogleConfigForm={(updater) =>
							setGoogleConfigForm((current) => updater(current))
						}
						googleForm={googleForm}
						setGoogleForm={(updater) =>
							setGoogleForm((current) => updater(current))
						}
						...
						onShopifyOrderAttributionBackfill={
							handleShopifyOrderAttributionBackfill
						}
						onOrderAttributionBackfillRefresh={() => {
							const jobId =
								orderAttributionBackfillJob.data?.jobId ??
								readStoredOrderAttributionBackfillJobId();
							if (jobId) {
								void loadOrderAttributionBackfillJob(jobId, {
									preserveData: true,
								});
							}
						}}
