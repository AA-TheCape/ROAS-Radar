...
gcloud run jobs deploy "$ATTRIBUTION_QA_RETENTION_JOB_NAME" \
  --project="$GCP_PROJECT_ID" \
  --region="$GCP_REGION" \
  --image="$IMAGE_URI" \
  --service-account="$RETENTION_SA" \
  ...
  --tasks=1 \
  --command=npm \
  --args=run,attribution-qa:retention:start \
  --set-env-vars="NODE_ENV=production,DATABASE_POOL_MIN=0,DATABASE_POOL_MAX=1,DATABASE_SSL=false,ATTRIBUTION_QA_RETENTION_DAYS=$ATTRIBUTION_QA_RETENTION_DAYS,ATTRIBUTION_QA_RETENTION_BATCH_SIZE=$ATTRIBUTION_QA_RETENTION_BATCH_SIZE,ATTRIBUTION_QA_RETENTION_MAX_BATCHES=$ATTRIBUTION_QA_RETENTION_MAX_BATCHES" \
  --set-secrets="DATABASE_URL=DATABASE_URL:latest" \
  --set-cloudsql-instances="$CLOUD_SQL_CONNECTION_NAME"
ensure_job_invoker "$ATTRIBUTION_QA_RETENTION_JOB_NAME" "serviceAccount:$SCHEDULER_INVOKER_SA"
ensure_job_invoker "$ATTRIBUTION_QA_RETENTION_JOB_NAME" "serviceAccount:$WORKER_SA"
...
echo "Configuring attribution QA retention scheduler job $ATTRIBUTION_QA_RETENTION_SCHEDULER_JOB_NAME"
