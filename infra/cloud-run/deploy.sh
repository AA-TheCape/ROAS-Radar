...
MMM_BASELINE_SA="$MMM_BASELINE_JOB_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
...
upsert_scheduler_job() {
  ...
  MAX_BACKOFF="${8:-1800s}"
  MAX_DOUBLINGS="${9:-1}"
  TIME_ZONE="${10:-$ADS_SYNC_TIME_ZONE}"
...
echo "Deploying MMM baseline training job $MMM_BASELINE_JOB_NAME"
gcloud run jobs deploy "$MMM_BASELINE_JOB_NAME" \
  --project="$GCP_PROJECT_ID" \
  --region="$GCP_REGION" \
  --image="$IMAGE_URI" \
  --service-account="$MMM_BASELINE_SA" \
  --cpu=1 \
  --memory=512Mi \
  --max-retries=1 \
  --task-timeout=1800 \
  ...
  --set-env-vars="^@^NODE_ENV=production@DATABASE_POOL_MIN=0@DATABASE_POOL_MAX=1@DATABASE_SSL=false@MMM_BASELINE_LOOKBACK_DAYS=$MMM_BASELINE_LOOKBACK_DAYS@MMM_BASELINE_LAG_DAYS=$MMM_BASELINE_LAG_DAYS@MMM_BASELINE_SUBMITTED_BY=$MMM_BASELINE_SUBMITTED_BY@MMM_BASELINE_ATTRIBUTION_MODEL=${MMM_BASELINE_ATTRIBUTION_MODEL:-last_touch}@MMM_BASELINE_MAX_SEGMENTS=${MMM_BASELINE_MAX_SEGMENTS:-8}@MMM_BASELINE_ADSTOCK_DECAY=${MMM_BASELINE_ADSTOCK_DECAY:-0.5}@MMM_BASELINE_RIDGE_LAMBDA=${MMM_BASELINE_RIDGE_LAMBDA:-1}@MMM_BASELINE_HOLDOUT_RATIO=${MMM_BASELINE_HOLDOUT_RATIO:-0.2}" \
  --set-secrets="DATABASE_URL=DATABASE_URL:latest" \
  --set-cloudsql-instances="$CLOUD_SQL_CONNECTION_NAME"
ensure_job_invoker "$MMM_BASELINE_JOB_NAME" "serviceAccount:$SCHEDULER_INVOKER_SA"
ensure_job_invoker "$MMM_BASELINE_JOB_NAME" "serviceAccount:$WORKER_SA"
...
echo "Configuring MMM baseline scheduler job $MMM_BASELINE_SCHEDULER_JOB_NAME"
upsert_scheduler_job \
  "$MMM_BASELINE_SCHEDULER_JOB_NAME" \
  "$MMM_BASELINE_JOB_NAME" \
  "$MMM_BASELINE_SCHEDULE" \
