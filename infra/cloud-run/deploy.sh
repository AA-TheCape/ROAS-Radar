...
# The retention job only needs the same database + secret access already granted to the
# worker runtime, and staging does not currently grant the deployer actAs on the dedicated
# retention service account. Reuse the working worker identity to keep deploys convergent.
RETENTION_SA="$WORKER_SA"
