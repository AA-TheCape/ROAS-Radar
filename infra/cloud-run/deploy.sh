...
META_ADS_DETERMINISTIC_SA="$META_ADS_DETERMINISTIC_JOB_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
GOOGLE_ADS_SA="$GOOGLE_ADS_JOB_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
GA4_INGESTION_SA="$GA4_INGESTION_JOB_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
CAMPAIGN_METADATA_BACKFILL_SA="$CAMPAIGN_METADATA_BACKFILL_JOB_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
SHOPIFY_ORDER_REIMPORT_SA="$SHOPIFY_ORDER_REIMPORT_JOB_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
ORDER_ATTRIBUTION_BACKFILL_SA="$ORDER_ATTRIBUTION_BACKFILL_JOB_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
SHOPIFY_ATTRIBUTION_RECOVERY_SA="$SHOPIFY_ATTRIBUTION_RECOVERY_JOB_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
GA4_FALLBACK_RECOVERY_SA="$GA4_FALLBACK_RECOVERY_JOB_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
DEAD_LETTER_REPLAY_SA="$DEAD_LETTER_REPLAY_JOB_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
...
MMM_BAYESIAN_SA="$MMM_BAYESIAN_JOB_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
SCHEDULER_INVOKER_SA="$SCHEDULER_INVOKER_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"
DEPLOYER_SA="$DEPLOYER_SERVICE_ACCOUNT_NAME@$GCP_PROJECT_ID.iam.gserviceaccount.com"

META_ADS_SECRET_FLAGS="META_ADS_APP_SECRET=META_ADS_APP_SECRET:latest,META_ADS_ENCRYPTION_KEY=META_ADS_ENCRYPTION_KEY:latest"
if [ -n "${META_ADS_METADATA_ACCESS_TOKEN_SECRET_NAME:-}" ]; then
  META_ADS_SECRET_FLAGS="$META_ADS_SECRET_FLAGS,META_ADS_METADATA_ACCESS_TOKEN=$META_ADS_METADATA_ACCESS_TOKEN_SECRET_NAME:latest"
fi
COMMON_SECRET_FLAGS="DATABASE_URL=DATABASE_URL:latest,REPORTING_API_TOKEN=REPORTING_API_TOKEN:latest,SHOPIFY_WEBHOOK_SECRET=SHOPIFY_WEBHOOK_SECRET:latest,SHOPIFY_APP_API_KEY=SHOPIFY_APP_API_KEY:latest,SHOPIFY_APP_API_SECRET=SHOPIFY_APP_API_SECRET:latest,SHOPIFY_APP_ENCRYPTION_KEY=SHOPIFY_APP_ENCRYPTION_KEY:latest,$META_ADS_SECRET_FLAGS,GOOGLE_ADS_ENCRYPTION_KEY=GOOGLE_ADS_ENCRYPTION_KEY:latest"
...
GA4_BIGQUERY_ENV_VARS="GA4_BIGQUERY_ENABLED=true@GA4_BIGQUERY_PROJECT_ID=$GA4_BIGQUERY_PROJECT_ID@GA4_BIGQUERY_LOCATION=$GA4_BIGQUERY_LOCATION@GA4_BIGQUERY_DATASET=$GA4_BIGQUERY_DATASET@GA4_BIGQUERY_EVENTS_TABLE_PATTERN=$GA4_BIGQUERY_EVENTS_TABLE_PATTERN@GA4_BIGQUERY_INTRADAY_TABLE_PATTERN=$GA4_BIGQUERY_INTRADAY_TABLE_PATTERN@GA4_BIGQUERY_LOOKBACK_HOURS=${GA4_BIGQUERY_LOOKBACK_HOURS:-24}@GA4_BIGQUERY_BACKFILL_HOURS=${GA4_BIGQUERY_BACKFILL_HOURS:-168}@GOOGLE_ADS_TRANSFER_BIGQUERY_PROJECT_ID=$GOOGLE_ADS_TRANSFER_BIGQUERY_PROJECT_ID@GOOGLE_ADS_TRANSFER_BIGQUERY_LOCATION=$GOOGLE_ADS_TRANSFER_BIGQUERY_LOCATION@GOOGLE_ADS_TRANSFER_BIGQUERY_DATASET=$GOOGLE_ADS_TRANSFER_BIGQUERY_DATASET@GOOGLE_ADS_TRANSFER_TABLE_PATTERN=$GOOGLE_ADS_TRANSFER_TABLE_PATTERN@GOOGLE_ADS_TRANSFER_LOOKBACK_DAYS=${GOOGLE_ADS_TRANSFER_LOOKBACK_DAYS:-30}@GA4_LINKED_GOOGLE_ADS_CUSTOMER_IDS=${GA4_LINKED_GOOGLE_ADS_CUSTOMER_IDS:-}"
GA4_INGESTION_ENV_VARS="^@^NODE_ENV=production@DATABASE_POOL_MIN=0@DATABASE_POOL_MAX=1@DATABASE_SSL=false@GA4_INGESTION_REQUESTED_BY=$GA4_INGESTION_REQUESTED_BY@GA4_INGESTION_BATCH_SIZE=$GA4_INGESTION_BATCH_SIZE@GA4_INGESTION_MAX_RETRIES=$GA4_INGESTION_MAX_RETRIES@GA4_INGESTION_INITIAL_BACKOFF_SECONDS=$GA4_INGESTION_INITIAL_BACKOFF_SECONDS@GA4_INGESTION_MAX_BACKOFF_SECONDS=$GA4_INGESTION_MAX_BACKOFF_SECONDS@GA4_INGESTION_STALE_LOCK_MINUTES=$GA4_INGESTION_STALE_LOCK_MINUTES@$GA4_BIGQUERY_ENV_VARS"
...
ensure_job_invokers_csv() {
  ...
  for MEMBER in "$@"; do
    trimmed=$(printf '%s' "$MEMBER" | awk '{$1=$1; print}')
    if [ -n "$trimmed" ]; then
      ensure_job_invoker "$TARGET_JOB_NAME" "$trimmed"
    fi
  done
}
...
deploy_manual_job() {
  ...
  ensure_job_invoker "$JOB_NAME" "serviceAccount:$WORKER_SA"
  ensure_job_invoker "$JOB_NAME" "serviceAccount:$DEPLOYER_SA"
  ensure_job_invokers_csv "$JOB_NAME" "${MANUAL_JOB_INVOKER_MEMBERS:-}"
}
...
gcloud run jobs deploy "$META_ADS_JOB_NAME" \
  ...
  --set-secrets="DATABASE_URL=DATABASE_URL:latest,$META_ADS_SECRET_FLAGS" \
  --set-cloudsql-instances="$CLOUD_SQL_CONNECTION_NAME"
...
echo "Deploying Meta deterministic sync job $META_ADS_DETERMINISTIC_JOB_NAME"
gcloud run jobs deploy "$META_ADS_DETERMINISTIC_JOB_NAME" \
  --project="$GCP_PROJECT_ID" \
  --region="$GCP_REGION" \
  --image="$IMAGE_URI" \
  --service-account="$META_ADS_DETERMINISTIC_SA" \
  --cpu=1 \
  --memory=512Mi \
  --max-retries="$META_ADS_JOB_MAX_RETRIES" \
  --task-timeout="$META_ADS_JOB_TIMEOUT_SECONDS" \
  ...
  --tasks=1 \
  --command=npm \
  --args=run,meta-ads:deterministic:start \
  --set-env-vars="${COMMON_ENV_VARS}@DATABASE_POOL_MAX=$ADS_SYNC_DATABASE_POOL_MAX@META_ADS_DETERMINISTIC_SYNC_ENABLED=$META_ADS_DETERMINISTIC_SYNC_ENABLED@META_ADS_DETERMINISTIC_SYNC_INITIAL_LOOKBACK_DAYS=$META_ADS_DETERMINISTIC_SYNC_INITIAL_LOOKBACK_DAYS@META_ADS_DETERMINISTIC_SYNC_LOOKBACK_DAYS=$META_ADS_DETERMINISTIC_SYNC_LOOKBACK_DAYS" \
  --set-secrets="DATABASE_URL=DATABASE_URL:latest,$META_ADS_SECRET_FLAGS" \
  --set-cloudsql-instances="$CLOUD_SQL_CONNECTION_NAME"
ensure_job_invoker "$META_ADS_DETERMINISTIC_JOB_NAME" "serviceAccount:$SCHEDULER_INVOKER_SA"
ensure_job_invoker "$META_ADS_DETERMINISTIC_JOB_NAME" "serviceAccount:$WORKER_SA"
...
gcloud run jobs deploy "$GA4_INGESTION_JOB_NAME" \
  --project="$GCP_PROJECT_ID" \
  --region="$GCP_REGION" \
  --image="$IMAGE_URI" \
  --service-account="$GA4_INGESTION_SA" \
  --cpu=1 \
  --memory=512Mi \
  --max-retries=0 \
  --task-timeout=1800 \
  ...
  --tasks=1 \
  --command=npm \
  --args=run,ga4:ingest:start \
  --set-env-vars="$GA4_INGESTION_ENV_VARS" \
  --set-secrets="DATABASE_URL=DATABASE_URL:latest" \
  --set-cloudsql-instances="$CLOUD_SQL_CONNECTION_NAME"
ensure_job_invoker "$GA4_INGESTION_JOB_NAME" "serviceAccount:$SCHEDULER_INVOKER_SA"
ensure_job_invoker "$GA4_INGESTION_JOB_NAME" "serviceAccount:$WORKER_SA"
ensure_job_invokers_csv "$GA4_INGESTION_JOB_NAME" "${MANUAL_JOB_INVOKER_MEMBERS:-}"

echo "Deploying manual campaign metadata backfill job $CAMPAIGN_METADATA_BACKFILL_JOB_NAME"
deploy_manual_job \
  "$CAMPAIGN_METADATA_BACKFILL_JOB_NAME" \
  "$CAMPAIGN_METADATA_BACKFILL_SA" \
  "campaign-metadata:backfill:start" \
  "${COMMON_ENV_VARS}@DATABASE_POOL_MAX=1" \
  "DATABASE_URL=DATABASE_URL:latest,$META_ADS_SECRET_FLAGS,GOOGLE_ADS_ENCRYPTION_KEY=GOOGLE_ADS_ENCRYPTION_KEY:latest"

echo "Deploying manual Shopify order reimport job $SHOPIFY_ORDER_REIMPORT_JOB_NAME"
deploy_manual_job \
  "$SHOPIFY_ORDER_REIMPORT_JOB_NAME" \
  "$SHOPIFY_ORDER_REIMPORT_SA" \
  "shopify:reimport-orders:start" \
  "${COMMON_ENV_VARS}@DATABASE_POOL_MAX=1" \
  "DATABASE_URL=DATABASE_URL:latest,SHOPIFY_APP_API_KEY=SHOPIFY_APP_API_KEY:latest,SHOPIFY_APP_API_SECRET=SHOPIFY_APP_API_SECRET:latest,SHOPIFY_APP_ENCRYPTION_KEY=SHOPIFY_APP_ENCRYPTION_KEY:latest"

echo "Deploying manual order attribution backfill job $ORDER_ATTRIBUTION_BACKFILL_JOB_NAME"
deploy_manual_job \
  "$ORDER_ATTRIBUTION_BACKFILL_JOB_NAME" \
  "$ORDER_ATTRIBUTION_BACKFILL_SA" \
  "attribution:backfill-orders:start" \
  "${COMMON_ENV_VARS}@DATABASE_POOL_MAX=1" \
  "DATABASE_URL=DATABASE_URL:latest,SHOPIFY_APP_ENCRYPTION_KEY=SHOPIFY_APP_ENCRYPTION_KEY:latest"

echo "Deploying manual Shopify attribution recovery job $SHOPIFY_ATTRIBUTION_RECOVERY_JOB_NAME"
deploy_manual_job \
  "$SHOPIFY_ATTRIBUTION_RECOVERY_JOB_NAME" \
  "$SHOPIFY_ATTRIBUTION_RECOVERY_SA" \
  "attribution:recover-shopify-hints:start" \
  "${COMMON_ENV_VARS}@DATABASE_POOL_MAX=1" \
  "DATABASE_URL=DATABASE_URL:latest,SHOPIFY_APP_ENCRYPTION_KEY=SHOPIFY_APP_ENCRYPTION_KEY:latest"

echo "Deploying manual GA4 fallback recovery job $GA4_FALLBACK_RECOVERY_JOB_NAME"
deploy_manual_job \
  "$GA4_FALLBACK_RECOVERY_JOB_NAME" \
  "$GA4_FALLBACK_RECOVERY_SA" \
  "attribution:recover-ga4-fallback:start" \
  "${COMMON_ENV_VARS}@DATABASE_POOL_MAX=1" \
  "DATABASE_URL=DATABASE_URL:latest"

echo "Deploying manual dead-letter replay job $DEAD_LETTER_REPLAY_JOB_NAME"
deploy_manual_job \
  "$DEAD_LETTER_REPLAY_JOB_NAME" \
  "$DEAD_LETTER_REPLAY_SA" \
  "dead-letters:replay:start" \
  "NODE_ENV=production,DATABASE_POOL_MIN=0,DATABASE_POOL_MAX=1,DATABASE_SSL=false" \
  "DATABASE_URL=DATABASE_URL:latest"
...
echo "Deploying MMM Bayesian training job $MMM_BAYESIAN_JOB_NAME"
gcloud run jobs deploy "$MMM_BAYESIAN_JOB_NAME" \
  --project="$GCP_PROJECT_ID" \
  --region="$GCP_REGION" \
  --image="$IMAGE_URI" \
  --service-account="$MMM_BAYESIAN_SA" \
  --cpu="${MMM_BAYESIAN_JOB_CPU:-1}" \
  --memory="${MMM_BAYESIAN_JOB_MEMORY:-1Gi}" \
  --max-retries="${MMM_BAYESIAN_JOB_MAX_RETRIES:-0}" \
  --task-timeout="${MMM_BAYESIAN_JOB_TIMEOUT_SECONDS:-3600}" \
  --parallelism=1 \
  --tasks=1 \
  --command=npm \
  --args=run,mmm:train-bayesian:start \
  --set-env-vars="^@^NODE_ENV=production@DATABASE_POOL_MIN=0@DATABASE_POOL_MAX=1@DATABASE_SSL=false@MMM_BAYESIAN_LOOKBACK_DAYS=$MMM_BAYESIAN_LOOKBACK_DAYS@MMM_BAYESIAN_LAG_DAYS=$MMM_BAYESIAN_LAG_DAYS@MMM_BAYESIAN_SUBMITTED_BY=$MMM_BAYESIAN_SUBMITTED_BY@MMM_BAYESIAN_ATTRIBUTION_MODEL=$MMM_BAYESIAN_ATTRIBUTION_MODEL@MMM_BAYESIAN_FREEZE_ID=$MMM_BAYESIAN_FREEZE_ID@MMM_BAYESIAN_REFRESH_MART=${MMM_BAYESIAN_REFRESH_MART:-false}@MMM_BAYESIAN_MAX_CHANNELS=${MMM_BAYESIAN_MAX_CHANNELS:-12}@MMM_BAYESIAN_ADSTOCK_DECAY=${MMM_BAYESIAN_ADSTOCK_DECAY:-0.55}@MMM_BAYESIAN_SATURATION_HALF_SATURATION=${MMM_BAYESIAN_SATURATION_HALF_SATURATION:-1000}@MMM_BAYESIAN_SATURATION_SLOPE=${MMM_BAYESIAN_SATURATION_SLOPE:-1.4}@MMM_BAYESIAN_MEDIA_PRIOR_SD=${MMM_BAYESIAN_MEDIA_PRIOR_SD:-0.35}@MMM_BAYESIAN_GROUP_PRIOR_SD=${MMM_BAYESIAN_GROUP_PRIOR_SD:-0.5}@MMM_BAYESIAN_CONTROL_PRIOR_SD=${MMM_BAYESIAN_CONTROL_PRIOR_SD:-5}@MMM_BAYESIAN_POSTERIOR_CHAINS=${MMM_BAYESIAN_POSTERIOR_CHAINS:-4}@MMM_BAYESIAN_POSTERIOR_DRAWS=${MMM_BAYESIAN_POSTERIOR_DRAWS:-1000}@MMM_BAYESIAN_POSTERIOR_WARMUP_DRAWS=${MMM_BAYESIAN_POSTERIOR_WARMUP_DRAWS:-500}@MMM_BAYESIAN_HOLDOUT_RATIO=${MMM_BAYESIAN_HOLDOUT_RATIO:-0.2}@MMM_BAYESIAN_RANDOM_SEED=${MMM_BAYESIAN_RANDOM_SEED:-}" \
  --set-secrets="DATABASE_URL=DATABASE_URL:latest" \
  --set-cloudsql-instances="$CLOUD_SQL_CONNECTION_NAME"
ensure_job_invoker "$MMM_BAYESIAN_JOB_NAME" "serviceAccount:$SCHEDULER_INVOKER_SA"
ensure_job_invoker "$MMM_BAYESIAN_JOB_NAME" "serviceAccount:$WORKER_SA"
...
echo "Configuring Meta deterministic scheduler job $META_ADS_DETERMINISTIC_SCHEDULER_JOB_NAME"
upsert_scheduler_job \
  "$META_ADS_DETERMINISTIC_SCHEDULER_JOB_NAME" \
  "$META_ADS_DETERMINISTIC_JOB_NAME" \
  "$META_ADS_DETERMINISTIC_SYNC_SCHEDULE" \
...
echo "Configuring GA4 ingestion scheduler job $GA4_INGESTION_SCHEDULER_JOB_NAME"
configure_metadata_scheduler \
  "$GA4_INGESTION_JOB_NAME" \
  "$GA4_INGESTION_SCHEDULER_JOB_NAME" \
  "$GA4_INGESTION_SCHEDULE" \
  "$GA4_INGESTION_SCHEDULER_TIME_ZONE" \
  "${GA4_INGESTION_SCHEDULER_ENABLED:-true}"
...
echo "Configuring MMM Bayesian scheduler job $MMM_BAYESIAN_SCHEDULER_JOB_NAME"
upsert_scheduler_job \
  "$MMM_BAYESIAN_SCHEDULER_JOB_NAME" \
  "$MMM_BAYESIAN_JOB_NAME" \
  "$MMM_BAYESIAN_SCHEDULE" \
  "$MMM_BAYESIAN_SCHEDULER_PAUSED" \
  "${MMM_BAYESIAN_SCHEDULER_ATTEMPT_DEADLINE:-3600s}" \
  "${MMM_BAYESIAN_SCHEDULER_MAX_RETRY_ATTEMPTS:-0}" \
  "${MMM_BAYESIAN_SCHEDULER_MIN_BACKOFF:-300s}" \
  "${MMM_BAYESIAN_SCHEDULER_MAX_BACKOFF:-3600s}" \
  "${MMM_BAYESIAN_SCHEDULER_MAX_DOUBLINGS:-1}" \
  "$MMM_BAYESIAN_TIME_ZONE"
